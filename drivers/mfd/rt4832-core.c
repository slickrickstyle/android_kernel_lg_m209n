 ...
 */
#ifdef CONFIG_LGD_SSD2068_FWVGA_VIDEO_PANEL
int rt4832_periodic_ctrl(int enable)
{
	...
	if (enable) {
		...
		if(lge_get_mfts_mode()){
			...
			ret |= regmap_write(rt4832_base->regmap, 0x0D, 0xA7);
			mdelay(1);
			ret |= regmap_write(rt4832_base->regmap, 0x0E, 0x64);
			ret |= regmap_write(rt4832_base->regmap, 0x0F, 0x64);
		} else {
		//Normal Mode
			ret = regmap_write(rt4832_base->regmap, 0x0C, 0x48);
			ret |= regmap_write(rt4832_base->regmap, 0x0D, 0xA7);
			ret |= regmap_write(rt4832_base->regmap, 0x0E, 0x64);
			ret |= regmap_write(rt4832_base->regmap, 0x0F, 0x64);
		}
		...
		//Toggle Mode
			...
			ret |= regmap_write(rt4832_base->regmap, 0x0D, 0x28);
			mdelay(1);
			ret |= regmap_write(rt4832_base->regmap, 0x0E, 0x28);
			ret |= regmap_write(rt4832_base->regmap, 0x0F, 0x28);
		// Always On Mode
		} else {
			ret = regmap_write(rt4832_base->regmap, 0x0D, 0x28);
			ret |= regmap_write(rt4832_base->regmap, 0x0E, 0x28);
			ret |= regmap_write(rt4832_base->regmap, 0x0F, 0x28);
			ret |= regmap_write(rt4832_base->regmap, 0x08, 0x0C);
			if(panel_esd_state) {
				regmap_write(rt4832_base->regmap, 0x0C, 0x40);
				regmap_write(rt4832_base->regmap, 0x0C, 0x00);
			}
		}
	}
	...
	if (ret)
		...
		pr_info("RT4832 mode is %s\n", enable?"Normal" : lge_get_mfts_mode()? "Toggle" : "AlwaysOn");
...
int rt4832_i2c_ctrl(u8 addr, u8 data)
{
	...
	ret = regmap_write(rt4832_base->regmap, addr, data);

	if (ret) {
		pr_err("%s write fail!!\n", __func__);
	} else {
		pr_debug("RT4832 i2c wite addr: %x, data: %x\n", addr, data);
	}

	return ret;
}
EXPORT_SYMBOL_GPL(rt4832_i2c_ctrl);
