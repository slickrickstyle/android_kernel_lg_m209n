...
enum LG_LED_PTRNS {
	...
	ID_LED_ON_160ms_IN_2000ms,			/* 920ms off => 160ms => 920ms off repeat */
#else
...
#elif defined(CONFIG_MSM8909_K6P)
	[ID_LED_ON_160ms_IN_2000ms] =                   {.brightness_val = 10, .period_in_msec = 2000, .repeat = 0},
#else
	...
	const char *buf, size_t count)
{
	...
	switch (pattern_id) {
		...
		case ID_URGENT_CALL_MISSED_NOTI:
			...
			led->ptrn_id = ID_LED_ON_160ms_IN_2000ms; /* one color LED model only */
#else
