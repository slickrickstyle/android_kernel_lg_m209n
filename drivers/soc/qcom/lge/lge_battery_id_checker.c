...
bool is_lge_battery_valid(void)
{
	...
	if (lge_battery_info == BATT_ID_DS2704_N ||
			...
			lge_battery_info == BATT_ID_ISL6296A_N ||
			lge_battery_info == BATT_ID_ISL6296A_L ||
			lge_battery_info == BATT_ID_ISL6296A_C)
		return true;
...
static int __init battery_information_setup(char *batt_info)
{
	...
	if (!strcmp(batt_info, "DS2704_N"))
		...
		lge_battery_info = BATT_ID_ISL6296_C;
	else if (!strcmp(batt_info, "ISL6296A_N"))
		lge_battery_info = BATT_ID_ISL6296A_N;
	else if (!strcmp(batt_info, "ISL6296A_L"))
		lge_battery_info = BATT_ID_ISL6296A_L;
	else if (!strcmp(batt_info, "ISL6296A_C"))
		lge_battery_info = BATT_ID_ISL6296A_C;
