...
#ifdef CONFIG_MACH_MSM8909_LV1_GLOBAL_COM
...
static void init_monitor_work(struct work_struct *work)
{
	...
	queue_delayed_work(monitor_wq, &monitor_dd->monitor_work_struct,
								delay_time);
...
#if defined(CONFIG_MSM8909_LV0) || defined(CONFIG_MSM8909_LV1)
/* [LGE_UPDATE_S] */
static long vs_temp = 25;

extern int lge_get_xo_therm(void);
extern int lge_get_pmic_therm(void);

extern int lgcc_is_charger_present(void);

#ifdef CONFIG_MSM8909_LV0
#define XO_WEIGHT	110
#define PMIC_WEIGHT	25
#define VS_TEMP_CONST	20
#else
#define XO_WEIGHT    69
#define PMIC_WEIGHT  11
#define WITH_TA_CONST   11
#define WITHOUT_TA_CONST 41
#endif
...
static int get_lge_vs_temp(char *buffer, struct kernel_param *kp)
{
	...
	xo_temp = lge_get_xo_therm() * XO_WEIGHT;
	pmic_temp = lge_get_pmic_therm() * PMIC_WEIGHT;
#if defined(CONFIG_MSM8909_LV1) && defined(CONFIG_MACH_MSM8909_LV1_GLOBAL_COM)
	if (lge_get_board_revno() <= HW_REV_A) {
		is_charging = 1;
	} else {
		is_charging = lgcc_is_charger_present();
	}
#elif defined(CONFIG_MSM8909_LV1) && !defined(CONFIG_MACH_MSM8909_LV1_GLOBAL_COM)
	is_charging = lgcc_is_charger_present();
#elif defined(CONFIG_MSM8909_LV0)
    is_charging = lgcc_is_charger_present();
#else
    is_charging = 0;
#endif

    if((xo_temp - pmic_temp) != 0)
    {
#ifdef CONFIG_MSM8909_LV0
	vs_temp = (xo_temp / 100L) - (pmic_temp / 100000L) + (VS_TEMP_CONST / 10L);
#else
	if(is_charging)
	{
			vs_temp = (xo_temp / 100L) + (pmic_temp / 100000L) + (WITH_TA_CONST / 10L);
		}
		else
		{
			vs_temp = (xo_temp / 100L) + (pmic_temp / 100000L) + (WITHOUT_TA_CONST / 10L);
	}
		//vs_temp /= 1000L;
#endif
    }

	return sprintf(buffer, "%ld",vs_temp );
}
