...
static ssize_t qfprom_featureid_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  u32 ret = 0, read_value = 0;

  read_value = qfprom_read(feature_id.addr) & feature_id.lsb;
  ret = read_value >> FEATURE_ID_SHIFT;
  printk(KERN_INFO "[QFUSE]%s : Read_value %x\n", __func__, ret);
  return sprintf(buf, "0x%08X\n", ret);
}
static DEVICE_ATTR(fid, S_IWUSR | S_IRUGO, qfprom_featureid_show, NULL);
...
static u32 qfprom_secdat_read(void)
{
  ...
  fp = file_open_root(root.dentry, root.mnt, SEC_PATH, O_RDONLY);
  path_put(&root);
