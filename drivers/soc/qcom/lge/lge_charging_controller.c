...
static void lge_monitor_batt_temp_work(struct work_struct *work){
	...
	if (charger_type == POWER_SUPPLY_TYPE_USB_DCP ) {
		the_controller->chg_current_max = 1800;
	} else if (charger_type == POWER_SUPPLY_TYPE_USB_HVDCP) {
		the_controller->chg_current_max = 2400;
	} else {
...
#elif defined CONFIG_LGE_PM_THERMAL_VTS
	the_controller->usb_psy->get_property(
			the_controller->usb_psy, POWER_SUPPLY_PROP_TYPE, &ret);
	if (ret.intval == POWER_SUPPLY_TYPE_USB) {
		the_controller->chg_current_max = 500;
	} else {
		the_controller->usb_psy->get_property(
			the_controller->usb_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &ret);
		the_controller->chg_current_max = ret.intval / 1000;
...
static int lge_charging_controller_probe(struct platform_device *pdev)
{
	...
	if(lge_get_factory_boot()) {
		pr_info("[LGCC] Skip a charging controller, factory cable connected\n");
		return 0;
	}
#endif
