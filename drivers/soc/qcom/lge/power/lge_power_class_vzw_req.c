...
#endif
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_CHARGER_SLEEP
	struct wake_lock llk_wake_lock;
#endif
};

static void lge_vzw_set_vzw_chg_work(struct work_struct *work)
{
	struct lge_vzw *chip = container_of(work, struct lge_vzw, set_vzw_chg_work);
...
{
	struct lge_vzw *chip = container_of(work, struct lge_vzw, llk_work);
	int prev_chg_enable = chip->charging_enable;

	if (chip->chg_present) {
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_CHARGER_SLEEP
		if (!wake_lock_active(&chip->llk_wake_lock))
			wake_lock(&chip->llk_wake_lock);
#endif
		...
		if (chip->charging_enable != prev_chg_enable) {
			pr_debug("lge_power_changed in LLK_mode.\n");
			pr_info("%s Charging by LLK_MODE.\n",
					chip->charging_enable ? "Start" : "Stop");
			lge_power_changed(&chip->lge_vzw_lpc);
		}
	}
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_CHARGER_SLEEP
		...
		if (wake_lock_active(&chip->llk_wake_lock))
			wake_unlock(&chip->llk_wake_lock);
	}
#endif
}
...
static void lge_vzw_external_power_changed(struct lge_power *lpc)
{
	...
	struct lge_vzw *chip = container_of(lpc, struct lge_vzw, lge_vzw_lpc);
	...
	if (!chip->batt_psy) {
		...
		if (!chip->batt_psy) {
			pr_err("battery power_supply get failed\n");
			return;
		}
	}
	if (!chip->chg_psy) {
#ifdef CONFIG_LGE_PM_CHARGING_RT5058_CHARGER
		chip->chg_psy = power_supply_get_by_name("rt5058-charger");
#else
		/* here to add another charger psy */
#endif
		if (!chip->chg_psy) {
			pr_err("external charger power_supply get failed\n");
			return;
		}
	}
	if (!chip->batt_psy && !chip->chg_psy) {
		pr_err("%s : battery or charger is not yet ready\n", __func__);
	} else {
		if (chip->current_settled >= 0) {
			rc = chip->chg_psy->get_property(chip->chg_psy,
		...
		if (chip->store_demo_enabled == 1) {
			...
			if (rc) {
				...
				pr_info("capacity : %d, charging eanble : %d\n",
						chip->capacity, chip->charging_enable);
			...
			const union lge_power_propval *val)
{
	...
	case LGE_POWER_PROP_STORE_DEMO_ENABLED:
		...
		lge_vzw_external_power_changed(&chip->lge_vzw_lpc);
		break;
...
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_CHARGER_SLEEP
	wake_lock_init(&chip->llk_wake_lock, WAKE_LOCK_SUSPEND,
			"lpc_vzw_llk_wake_lock");
#endif
...
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_CHARGER_SLEEP
	wake_lock_destroy(&chip->llk_wake_lock);
#endif
