...
#if defined (CONFIG_TOUCHSCREEN_MIT200)
void (*rpr0521_proximity_change_cb)(int);
/* only one callback is maintained. may change it to list of callbacks */
void rpr0521_register_proximity_change_callback (void (*callback) (int))
{
    rpr0521_proximity_change_cb = callback;
}
#endif
...
static int sensor_parse_dt(struct device *dev, PS_ALS_DATA *ps_als)
{
	...
	struct sensor_dt_to_pdata_map map[] = {
		...
		{"Rohm,dark_threshold", &ps_als->config.alsth_low, DT_SUGGESTED, DT_U32, PS_ALS_SET_ALS_TL},
		{"Rohm,bright_threshold", &ps_als->config.alsth_upper, DT_SUGGESTED, DT_U32, PS_ALS_SET_ALS_TH},
		/*D1-5T-SW-INPUT@lge.com*/
...
#if defined (CONFIG_TOUCHSCREEN_MIT200)
		if(rpr0521_proximity_change_cb) {
			rpr0521_proximity_change_cb(true);
		}
#endif
	}
...
#if defined (CONFIG_TOUCHSCREEN_MIT200)
		if(rpr0521_proximity_change_cb) {
			rpr0521_proximity_change_cb(false);
		}
#endif
	}
	else
...
static int update_als_value(unsigned short new_als_data0, unsigned short new_als_data1)
{
	...
	else if(new_als_data0 < ps_als_ginfo->config.init_data.alsth_low) {	// current state is dark
...
static int make_init_data(PS_ALS_DATA *ps_als)
{
	...
	ps_als->config.init_data.alsth_upper = ps_als->config.alsth_upper;
	ps_als->config.init_data.alsth_low = ps_als->config.alsth_low;
