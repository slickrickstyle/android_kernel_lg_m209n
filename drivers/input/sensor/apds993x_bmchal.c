...
#define APDS9930_SENSOR_DEBUG 1
#if APDS9930_SENSOR_DEBUG
struct debug_reg {
	char  *name;
	u8  reg;
};
...
static void apds993x_change_ps_threshold(struct i2c_client *client)
{
	struct apds993x_data *data = i2c_get_clientdata(client);
	 ...
	 apds993x_set_pers(client, 0x23);    // Change proximity persistence value from 3 to 2(0x33 -> 0x23). To reduce AP wake up time
#else
...
//	pr_debug("%s: status = 0x%x, enable: 0x%x, ch0data: %d\n", __func__, status, enable, ch0data);
	...
	//	apds993x_change_als_threshold(client);
		...
		pr_info("%s : ch0data = %d, ps_data = %d, pilt = %d, piht = %d, data->ps_detection = %d, check_sunlight = %d, als_atime_index = %d\n",
			__func__, ch0data, data->ps_data, data->pilt, data->piht, data->ps_detection, check_sunlight, data->als_atime_index);
...
out:
	mutex_unlock(&data->op_lock);
        printk("%s: apds993x_enable_als_sensor... i2c error\n", __func__);
	return -1;
}
...
static int apds993x_enable_ps_sensor(struct i2c_client *client, int val)
{
	...
	if (val == 1) {
		...
		if (data->enable_ps_sensor==0) {
			...
			if (data->cross_talk <= 0 || data->cross_talk >= 1000) {
				data->cross_talk = DEFAULT_CROSS_TALK;
				pr_info("%s: set cross talk as default value\n", __func__);
			}
...
out:
	mutex_unlock(&data->op_lock);
        printk("%s: apds993x_enable_ps_sensor... i2c error \n", __func__);
	return -1;
}
...
static int sensor_regulator_configure(struct apds993x_data *data, bool on)
{
	...
	if (regulator_count_voltages(pdata->vcc_ana) > 0) {
		rc = regulator_set_voltage(pdata->vcc_ana,
...
#ifdef APDS9930_REGULATOR_ALAWAY_ON
	return 0;
#endif
		...
		struct apds993x_platform_data *pdata)
{
	...
	{"avago,ps_threshold",			&pdata->prox_threshold,			DT_REQUIRED,		DT_U32,		0},
	{"avago,ps_hysteresis_threshold",	&pdata->prox_hsyteresis_threshold,	DT_REQUIRED,		DT_U32,		0},
	{"avago,ps_pulse",			&pdata->prox_pulse,			DT_REQUIRED,		DT_U32,		0},
	{"avago,ps_pgain",			&pdata->prox_gain,			DT_REQUIRED,		DT_U32,		0},
	{"avago,als_B",				&pdata->als_B,				DT_REQUIRED,		DT_U32,		0},
	{"avago,als_C",				&pdata->als_C,				DT_REQUIRED,		DT_U32,		0},
	{"avago,als_D",				&pdata->als_D,				DT_REQUIRED,		DT_U32,		0},
	{"avago,ga_value",			&pdata->ga_value,			DT_REQUIRED,		DT_U32,		0},
	{"avago,vdd_ana_supply_min",		&pdata->vdd_ana_supply_min, 		DT_SUGGESTED,		DT_U32, 	2850000},
	{"avago,vdd_ana_supply_max",		&pdata->vdd_ana_supply_max, 		DT_SUGGESTED,		DT_U32, 	2850000},
	{"avago,vdd_ana_load_ua",		&pdata->vdd_ana_load_ua,		DT_SUGGESTED,		DT_U32, 	15000},
	{"avago,vddio_dig_supply_min",		&pdata->vddio_dig_supply_min,		DT_SUGGESTED,		DT_U32, 	1800000},
	{"avago,vddio_dig_supply_max",		&pdata->vddio_dig_supply_max,		DT_SUGGESTED,		DT_U32, 	1800000},
	{"avago,vddio_dig_load_ua", 		&pdata->vddio_dig_load_ua,		DT_SUGGESTED,		DT_U32, 	10000},
	{"avago,vddio_i2c_supply_min",		&pdata->vddio_i2c_supply_min,		DT_SUGGESTED,		DT_U32, 	1800000},
	{"avago,vddio_i2c_supply_max",		&pdata->vddio_i2c_supply_max,		DT_SUGGESTED,		DT_U32, 	1800000},
	{"avago,vddio_i2c_load_ua", 		&pdata->vddio_i2c_load_ua,		DT_SUGGESTED,		DT_U32, 	10000},
	{NULL			, NULL				  , 0			, 0 	 ,	0},
	};
