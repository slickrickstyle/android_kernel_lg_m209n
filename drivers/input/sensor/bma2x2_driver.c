...
#ifdef BMA2X2_ACCEL_CALIBRATION
/* calibration tilt limit against x/y axis pre-calculated by "RAW_VALUE(8192) * sin(15deg)" */
#define BMA2X2_FAST_CALIB_TILT_LIMIT	2120
...
static void bma2x2_work_func(struct work_struct *work)
{
	...
	//input_report_abs(bma2x2->input, ABS_RUDDER, (timespec_to_ns(&ts) & 0xffffffff));
		...
		const char *buf, size_t count)
{
	...
	//if (data == 40){
		/* I don't know well, if BMA255 can support 25Hz operation or not, it should be checked.
		 This is workarround to pass Accel 25Hz CTS operation. (25Hz -> 50hz)*/
		//SENSOR_LOG("delay (%ld)\n" , data);
		//data = 20;
	//}
...
#ifdef BMA2X2_ACCEL_CALIBRATION
	{"cal_range",					&pdata->cal_range,				DT_OPTIONAL,	DT_U32, 	BMA2X2_RANGE_2G},
#endif
	{NULL			, NULL				  , 0			, 0 	 ,	0}, 	
	};
