...
static void apds993x_work_handler(struct work_struct *work)
{
	...
	if(i2c_smbus_write_byte_data(client, CMD_BYTE|APDS993X_ENABLE_REG, 1) < 0){
		SENSOR_LOG("i2c_smbus_write_byte_data(client, CMD_BYTE|APDS993X_ENABLE_REG, 1) fail");
		return ;
	}

	SENSOR_DBG("status reg = %x, enable reg = %x", status, enable);
	...
	if(apds993x_set_enable(client, data->enable) < 0 ){
		SENSOR_LOG("apds993x_set_enable(client, data->enable) fail, data->enable=%d", data->enable);
		return ;
	}
}
...
static irqreturn_t apds993x_interrupt(int vec, void *info)
{
	...
	SENSOR_LOG("apds993x interrupt is handled!");
#ifdef APDS993X_PM_IRQ_SYNC
	if(atomic_read(&data->status) == APDS993X_STATUS_SUSPEND) {
				...
				struct device_attribute *attr, char *buf)
{
	...
	SENSOR_LOG("APDS993x_ENABLE_REG=%2d, enable_ps_sensor=%2d", rdata, data->enable_ps_sensor);
	return sprintf(buf, "%d\n", data->enable_ps_sensor);
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	SENSOR_LOG("APDS993x_ENABLE_REG=%2d, enable_als_sensor=%2d", rdata, data->enable_als_sensor);
	return sprintf(buf, "%d\n", data->enable_als_sensor);
}
