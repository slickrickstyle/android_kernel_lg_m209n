...
{
	struct k303b_status *stat = input_get_drvdata(input);
...
static int k303b_mag_input_init(struct k303b_status *stat)
{
...
#if 1 /* LGE_SENSOR_FACTORY_AAT */
	stat->input_dev_mag->dev.init_name = "lge_magnetometer";
#endif
	...
	input_set_abs_params(stat->input_dev_mag, ABS_THROTTLE,
				INT_MIN, INT_MAX, 0, 0);
	input_set_abs_params(stat->input_dev_mag, ABS_RUDDER,
				INT_MIN, INT_MAX, 0, 0);
...
static void __init async_k303b_init(void *data, async_cookie_t cookie)
{
    int ret = i2c_add_driver(&k303b_driver);
    if (ret)
...
{
    async_schedule(async_k303b_init, NULL);
    return 0;
}
