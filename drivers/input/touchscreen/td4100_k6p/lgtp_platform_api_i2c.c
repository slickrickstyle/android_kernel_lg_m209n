...
static int i2c_read(u8 *reg, int regLen, u8 *buf, int dataLen)
{
	...
	if( dataLen <= MAX_I2C_TRANSFER_SIZE )
	{
		...
		do{
			//TOUCH_ERR("pClient->addr : %x\n", pClient->addr);
			if(regLen == 0 || reg == NULL){
	...
	else
	{
		...
		if(regLen == 0 || reg == NULL){
			for (i = 0 ; i < msgCount-1 ; i++) {
				...
				msgs[i].buf = buf + MAX_I2C_TRANSFER_SIZE *(i);
			}
			if( remainedDataLen > 0 ) {
			msgs[msgCount-2].len = remainedDataLen;
			}
		}
		else{
		...
		do{
			if(regLen == 0 || reg == NULL){
				ret = i2c_msg_transfer(pClient, msgs, msgCount-1);
			}
			else{
...
int TD4100_I2C_Read ( struct i2c_client *client, u8 addr, u8 addrLen, u8 *rxbuf, int len )
{
	int ret = 0;
	u8 regValue = addr;

//TOUCH_FUNC();
	ret = i2c_read(&regValue, addrLen, rxbuf, len);
	if( ret == TOUCH_FAIL ) {
		if (printk_ratelimit()) {
			TOUCH_ERR("failed to read i2c ( reg = %d )\n", addr);
		}
...
int TD4100_I2C_Write ( struct i2c_client *client, u8 addr, u8 *txbuf, int len )
{
	int ret = 0;
	u8 regValue = addr;
...
int TD4100_I2C_Write_Byte ( struct i2c_client *client, u8 addr, u8 txbuf )
{
	int ret = 0;
	u8 regValue = addr;
	u8 data = txbuf;
...
//TOUCH_FUNC();
	ret = i2c_write(&regValue, 1, &data, 1);
	if( ret == TOUCH_FAIL ) {
		if (printk_ratelimit()) {
			TOUCH_ERR("failed to write i2c ( reg = %d )\n", addr);
		}
