...
#include <linux/input/unified_driver_4_k6p/lgtp_common_driver.h>
...
extern void FT8707_Set_Boot_TA(void);
#endif
...
void SetDriverState(TouchDriverData *pDriverData, TouchState newState)
{
	int ret = 0;
...
static void report_finger(TouchDriverData *pDriverData, TouchReadData *pReadData)
{
	...
	for (i = 0; i < pDriverData->mConfig.max_id; i++) {
	    if(pReadData->fingerData[i].status == FINGER_PRESSED){
            if(pDriverData->nextState != STATE_KNOCK_ON_ONLY &&
                pDriverData->nextState != STATE_KNOCK_ON_CODE &&
                pDriverData->nextState != STATE_OFF) {
                  ...
                  if(!(pressedFinger&reportedFinger)) {
                       if (lockscreen_stat) {
                           ...
                           TOUCH_LOG("[FINGER] PRESS<%d> x = [%4d], y = [%4d], z= [%3d]\n",
                               pReadData->fingerData[i].id,
                               pReadData->fingerData[i].x,
                               pReadData->fingerData[i].y,
                               pReadData->fingerData[i].pressure);
...
#if defined (TOUCH_MODEL_K7)
			if(((1 << pReadData->fingerData[i].id) & reportedFinger)) { /* filter unreported event */
			...
			pReadData->fingerData[i].status = FINGER_UNUSED; /* restore unreported event status */
		}
#else
				pReadData->fingerData[i].status = FINGER_UNUSED;
				input_mt_slot(pDriverData->input_dev, i);
...
#if defined (TOUCH_MODEL_K7)
	pDriverData->reportData.finger = (reportedFinger & (~releasedFinger)); /* Masking release data, prevent underflow */
#else
...
static void release_all_finger(TouchDriverData *pDriverData)
{
	...
	if (pDriverData->reportData.finger) {
		...
		readData.count = 0;
#if defined (TOUCH_MODEL_K7)
	pDriverData->reportData.finger = 0x01; /* for release all event, all the release event id is 0 */
#endif
...
static irqreturn_t TouchIrqHandler(int irq, void *dev_id)
{
	...
	TOUCH_LOG("TouchIrqHandler 1 ");
	if (pWorkTouch != NULL) {
		/* trigger work queue to process interrupt */
		TOUCH_LOG("TouchIrqHandler 2 pWorkTouch != NULL");
		//queue_delayed_work(touch_wq, pWorkTouch, 0); /* It will call "WqTouchIrqHandler()" */
		queue_delayed_work(touch_wq, pWorkTouch, 0); /* It will call "TouchIrqEventHandler()" */
	}
TOUCH_LOG("TouchIrqHandler 3 ");
	return IRQ_HANDLED;
}
//#elif defined(TOUCH_PLATFORM_MTK)
#elif defined(TOUCH_PLATFORM_QCT)
static irqreturn_t TouchIrqHandler(int irq, void *dev_id)
{
...
#if defined(TOUCH_MODEL_M4)
	if( m4_tddi_id == SECOND_MODULE ) {
		WRITE_SYSBUF(buf, ret,  "Product ID - FW Version [%s - %X.%02X / module id = %x] \n",
			TOUCH_DEVICE_NAME_M4, pDriverData->icFwInfo.isOfficial, pDriverData->icFwInfo.version, pDriverData->icFwInfo.modelID);
		return ret;
	}
#endif

	WRITE_SYSBUF(buf, ret,  "Product ID - FW Version [%s - %d.%02d / module id = %x] \n",
			TOUCH_DEVICE_NAME, pDriverData->icFwInfo.isOfficial, pDriverData->icFwInfo.version, pDriverData->icFwInfo.modelID);

#if defined(TOUCH_MODEL_K7)
	WRITE_SYSBUF(buf, ret,  "LOT ID - [%s] \n",pDriverData->icFwInfo.lot_id);
#endif
	return ret;
}
...
static ssize_t show_atcmd_fw_ver(TouchDriverData *pDriverData, char *buf)
{
	...
	WRITE_SYSBUF(buf, ret, "V%d.%02d (%X/%X/%X)\n",
...
static ssize_t show_get_int_gpio(TouchDriverData *pDriverData, char *buf)
{
	int ret = 0;
	int state = 0;
	...
	state = TouchReadGpioInterrupt();

	WRITE_SYSBUF(buf, ret, "int state = %d \n", state);
...
#if defined(TOUCH_MODEL_K6M) || defined (TOUCH_MODEL_K6P)
static ssize_t show_sd_temp(TouchDriverData *pDriverData, char *buf)
{
	int ret = 0;
	int channelStatus = 0;
	int rawStatus = 0;
...
static ssize_t show_lpwg_sd_temp(TouchDriverData *pDriverData, char *buf)
{
	int ret = 0;
	int lpwgStatus = 0;
	u8 *pBuf = NULL;
...
{
    int ret = 0;
	ret += snprintf(buf+ret, PAGE_SIZE - ret, "[%s] Start Default F/W Upgrade \n", __func__);

	mutex_lock(pMutexTouch);
	TOUCH_LOG("[%s] Start Default F/W Upgrade \n", __func__);
...
static LGE_TOUCH_ATTR(get_int_gpio, S_IRUGO | S_IWUSR, show_get_int_gpio, NULL);
#if defined(TOUCH_MODEL_K6M) || defined (TOUCH_MODEL_K6P)
static LGE_TOUCH_ATTR(sd_temp, S_IRUGO | S_IWUSR, show_sd_temp, NULL);
static LGE_TOUCH_ATTR(lpwg_sd_temp, S_IRUGO | S_IWUSR, show_lpwg_sd_temp, NULL);
#endif
...
static struct attribute *lge_touch_attribute_list[] = {
	...
	&lge_touch_attr_get_int_gpio.attr,
#if defined(TOUCH_MODEL_K6M) || defined (TOUCH_MODEL_K6P)
	&lge_touch_attr_sd_temp.attr,
	&lge_touch_attr_lpwg_sd_temp.attr,
#endif
	NULL,
};
...
static void SetNextState(int lcdState)
{
	...
	pDriverData = container_of(pWorkTouch,TouchDriverData,work_init);
    if(lcdState == 0){
...
#if defined(TOUCH_MODEL_M4) || defined (TOUCH_MODEL_K6M) || defined (TOUCH_MODEL_K6P)/* for TD4100 one chip */
void SetNextState_For_Touch(int lcdState)
{
	TouchDriverData *pDriverData = NULL;
	int check_lock = mutex_is_locked(pMutexTouch);
	int temp = lge_get_mfts_mode();

	TOUCH_LOG("4100__[MFTS_TEST] lge_get_mfts_mode()=%d \n",temp);
...
int touch_notify_call(int event,int data)
{
	...
	TOUCH_LOG("Block Touch notify for Charger Logo \n");
	return 0;
    }
    if(pMutexTouch == NULL) {
	TOUCH_LOG("pMutexTouch not Initialize. event = %d, data = %d \n",event, data);
	if((event == TA_EVENT_STATUS_CHANGE) && data == 4) {
	    FT8707_Set_Boot_TA();
	}
	return 0;
    }
    if(!is_probe) {
	TOUCH_LOG("touch not probed \n");
	return 0;
    }
#endif
...
{
	int ret = 0;
#if defined(TOUCH_MODEL_M2) || defined(TOUCH_MODEL_K6M)
	int idx = 0;
#endif
	...
	//INIT_DELAYED_WORK(&pDriverData->work_irq, TouchIrqEventHandler);
	...
	TouchEnableIrq();
#if defined (TOUCH_MODEL_K7) || defined (TOUCH_MODEL_K6M)// || defined (TOUCH_MODEL_K6P)
	TOUCH_LOG("[K7][K6M] F/W Upgrade Check. boot mode:%d\n",get_boot_mode());
	TOUCH_LOG("[K7][K6M] pDriverData->binFwInfo.version : %d  pDriverData->icFwInfo.version %d\n",pDriverData->binFwInfo.version, pDriverData->icFwInfo.version);
	
	
	if (pDriverData->binFwInfo.version != pDriverData->icFwInfo.version) {
		if(get_boot_mode() == RECOVERY_BOOT || get_boot_mode() == 15 || get_boot_mode() == 110){
			TOUCH_LOG("RECOVERY_BOOT or CHARGERLOGO_BOOT or DOWNLOAD_BOOT.\n");
			pDriverData->useDefaultFirmware = TOUCH_FALSE;
		}
		else{
			TOUCH_LOG("NOT RECOVERY_BOOT.\n");
			pDriverData->useDefaultFirmware = TOUCH_TRUE;
			queue_delayed_work(touch_wq, &pDriverData->work_upgrade, 0);
		}
	} else {
	...
	TOUCH_LOG("[K6P] F/W Upgrade Check. boot mode:%d\n",lge_get_boot_mode());
	TOUCH_LOG("[K6P] pDriverData->binFwInfo.version : %d  pDriverData->icFwInfo.version %d\n",pDriverData->binFwInfo.version, pDriverData->icFwInfo.version);

	TOUCH_LOG("[K6P] F/W Upgrade Check. boot reason:%d\n",lge_get_bootreason());
	TOUCH_LOG("[K6P] F/W Upgrade Check. boot recovery:%d\n",check_recovery_boot);
	if (pDriverData->binFwInfo.version != pDriverData->icFwInfo.version) {
		if(lge_get_laf_mode() == LGE_LAF_MODE_LAF || lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO || lge_get_bootreason() == HIDDEN_FACTORY_RESET || check_recovery_boot == LGE_RECOVERY_BOOT ){
			TOUCH_LOG("RECOVERY_BOOT or CHARGERLOGO_BOOT or DOWNLOAD_BOOT.\n");
			pDriverData->useDefaultFirmware = TOUCH_FALSE;
		}else{
			TOUCH_LOG("NOT RECOVERY_BOOT.\n");
...
static void async_touch_init(void *data, async_cookie_t cookie)
{
	...
	TOUCH_LOG("async_touch_init, idx = %d\n",idx);
...
#if defined ( TOUCH_MODEL_K6P )
	index = lcd_maker;
	TOUCH_LOG("[td4100] get_maker_id = %d \n",index);
	if(index == 1){//1 : LGD panel, 1: TOVIS panel
...
#if defined (TOUCH_MODEL_K7)
    if (lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO){
	TOUCH_LOG("Block Touch Init for Charger Logo \n");
	return 0;
    }
#endif
    async_schedule(async_touch_init, NULL);
...
#if defined (TOUCH_MODEL_K7)
    if (lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO){
	TOUCH_LOG("Block Touch exit for Charger Logo \n");
	return;
    }
#endif
