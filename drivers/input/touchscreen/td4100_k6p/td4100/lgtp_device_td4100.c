...
#include <linux/input/unified_driver_4_k6p/lgtp_device_td4100.h>
...
u8 LPWG_FAIL_REASON_ENABLE = 1;
#define MAX_NUM_OF_FAIL_REASON 2

#define MAX_PRESSURE			255
#define LPWG_BLOCK_SIZE			7
#define KNOCKON_DELAY			50	/* 700ms */
#define KNOCKCODE_DELAY			30	/* 300ms */
...
static const char defaultFirmware[] = "synaptics/m4/PLG595_V0.18_PR2311924_DS5.12.0.5.1039_20053118.img";
static const char defaultFirmwareRecovery[] = "synaptics/m4/PLG595_V0.18_PR2311924_DS5.12.0.5.1039_20053118.bin";
static const char defaultPanelSpec[] = "synaptics/m4/m2lte_limit.txt";

#elif defined ( TOUCH_MODEL_K6M )
static const char old_defaultFirmware[] = "synaptics/k6m/PLG596_V0.0A_PR2364646_DS5.12.2.0.1003_2005510A.img";
static const char old_defaultFirmwareRecovery[] = "synaptics/k6m/PLG596_V0.0A_PR2364646_DS5.12.2.0.1003_2005510A.bin";

static const char defaultFirmware[] = "synaptics/k6m/PLG596_V0.55_PR2364649_DS5.12.2.0.1003_20055155.img";
static const char defaultFirmwareRecovery[] = "synaptics/k6m/PLG596_V0.55_PR2364649_DS5.12.2.0.1003_20055155.bin";

static const char defaultPanelSpec[] = "synaptics/k6m/k6m_limit.txt";

#define TPD_I2C_ADDRESS 0x20
#define TPD_I2C_ADDRESS_SE 0x2C
#elif defined (TOUCH_MODEL_K6P)
static char old_defaultFirmware[] = "synaptics/k6p/PLG622-V0.01_PR2381660-DS5.12.3.3.1008_20053101.img";
static char old_defaultFirmwareRecovery[] = "synaptics/k6p/PLG622-V0.01_PR2381660-DS5.12.3.3.1008_20053101.bin";

static const char defaultFirmware[] = "synaptics/k6p/PLG622-V0.22_PR2415977_DS5.12.3.6.1005_20053116.img";
static const char defaultFirmwareRecovery[] = "synaptics/k6p/PLG622-V0.22_PR2415977_DS5.12.3.6.1005_20053116.bin";

static char defaultPanelSpec[] = "synaptics/k6p/k6p_limit.txt";
...
u8 cover_thr = 0x22;	//0.1328125, if cover is closed, set this threshold.
u8 sleep_mask = 0xFC;
#if defined(ENABLE_SWIPE_MODE)
...
extern int k6m_tovis_id;
#endif

#if defined(TOUCH_MODEL_K6P)
extern int k6p_tovis_id;
#endif
...
//static int TD4100_AccessRegister( int cmd, int reg, int *pValue);
...
static void synapitcs_change_ime_status(struct i2c_client *client, int ime_status)
{
	...
	if (ime_status) {
		TOUCH_LOG("IME on !!\n");
		udata[3] = 0x06;/*Drumming Acceleration Threshold*/
...
static int tci_control(struct i2c_client *client, int type, u8 value)
{
	...
	switch (type)
	{
		case REPORT_MODE_CTRL:
			...
			DO_SAFE(TouchReadReg(f12_info.ctrl_reg_addr[20], buffer, 3),error);	//addr 0x19
		...
		case INTERRUPT_DELAY_CTRL2:
			...
			ret = synaptics_ts_page_data_write_byte(client, LPWG_PAGE, LPWG_INTERRUPT_DELAY_REG2,
					value ? (buffer[0] = (KNOCKCODE_DELAY << 1) | 0x1) : (buffer[0] = 0));
			if( ret < 0 ) {
				return TOUCH_FAIL;
			}
			break;
...
static void TD4100_SetSensitivity(void)
{
	u8 filter[19] = {0};
	u8 finger_amp[7] = {0};
	u8 landing_max_frame = 0x01;	//lpwg = 1 (0x01)
	//u8 release_frame = 0x14;	//lpwg = 20 (0x14)
	u8 fin_amp = 0x40;	//lpwg = 0.25 (0x40)

	TOUCH_FUNC();

	/* Finger Landing Max Frames, Land Lock Fast Finger Release Frames */
	synaptics_ts_page_data_read(ds4_i2c_client, DEFAULT_PAGE, f12_info.ctrl_reg_addr[11], sizeof(filter), filter);
	filter[8] = filter[10] = landing_max_frame;
	//filter[17] = release_frame;
	synaptics_ts_page_data_write(ds4_i2c_client, DEFAULT_PAGE, f12_info.ctrl_reg_addr[11], sizeof(filter), filter);

	/* Finger /Small Finger Amplitude Threshold */
	synaptics_ts_page_data_read(ds4_i2c_client, DEFAULT_PAGE, f12_info.ctrl_reg_addr[15], sizeof(finger_amp), finger_amp);
	finger_amp[0] = finger_amp[1] = fin_amp;
	synaptics_ts_page_data_write(ds4_i2c_client, DEFAULT_PAGE, f12_info.ctrl_reg_addr[15], sizeof(finger_amp), finger_amp);
}
...
static void TD4100_ForceUpdate(void)
{
	...
	cmd = 0x04;
	synaptics_ts_page_data_write(ds4_i2c_client, ANALOG_PAGE, 0x3F, 1, &cmd);	//F54_ANALOG_CMD00
...
static int lpwg_control(struct i2c_client *client)
{
	...
	switch (ts->currState)
	{
		...
		case STATE_OFF:
			...
			tci_control(client, TCI_ENABLE_CTRL, 0);		// tci-1 disable
			tci_control(client, TCI_ENABLE_CTRL2, 0);		// tci-2 disable
#if defined(TOUCH_MODEL_K6M) || defined (TOUCH_MODEL_K6P)	
...
{
	int ret = 0;
#if defined ( TOUCH_MODEL_M4 ) || defined ( TOUCH_MODEL_K6M ) || defined (TOUCH_MODEL_K6P)
	return ret;
#endif
...
static ssize_t show_updated(TouchDriverData *pDriverData, char *buf)
{
...
static ssize_t store_swipe_mode(sTouchDriverData *pDriverData,
	const char *buf, size_t count)
{
	int value = 0;
...
static ssize_t store_cover_thr(TouchDriverData *pDriverData,
	const char *buf, size_t count)
{
	//int ret = 0;
	int thr = 0;
...
static ssize_t show_cover_thr(TouchDriverData *pDriverData, char *buf)
{
	int ret = 0;
...
static ssize_t store_sleep_mask(TouchDriverData *pDriverData,
	const char *buf, size_t count)
{
	//int ret = 0;
	int mask = 0;
...
static ssize_t show_sleep_mask(TouchDriverData *pDriverData, char *buf)
{
	int ret = 0;
...
int synaptics_ts_fw_recovery(struct synaptics_ts_data* ts)
{
	...
	if(ts->ubootloader_mode) {
		...
		if( k6p_tovis_id == 1 )
			...
			memcpy(path, old_defaultFirmwareRecovery, sizeof(path));
...
void write_time_log(char *filename, char *data, int data_include)
{
	...
	if (filename == NULL){
		...
		if(boot_mode != BOOT_NORMAL){
			...
			TOUCH_LOG("%s /data/touch/touch_self_test.txt\n",__func__);
		}else{
			fname = "/mnt/sdcard/touch_self_test.txt";
			TOUCH_LOG("%s /mnt/sdcard/touch_self_test.txt\n",__func__);
		}
#else
		fname = "/mnt/sdcard/touch_self_test.txt";
#endif
...
static void TD4100_SetCoverStatus(void)
{
	...
	if(QCoverClose){
		TOUCH_LOG( "%s : Cover Closed. tTouchWriteByteReg(0x1e, 0x02) \n" , __func__);
		TouchWriteByteReg(0x1e, 0x02);
	} else {
		TOUCH_LOG( "%s : Cover Open. tTouchWriteByteReg(0x1e, 0x00) \n" , __func__);
		TouchWriteByteReg(0x1e, 0x00);
	}
...
static int TD4100_GetBinFirmwareInfo( char *pFilename, TouchFirmwareInfo *pFwInfo)
{
	...
	if( pFilename == NULL ) {
		...
		if( k6p_tovis_id == 1 ) {
			...
			pFwFilename = (char *)old_defaultFirmware;
		}
...
static int TD4100_UpdateFirmware(  char *pFilename)
{
	...
	if((strncmp(fw_product_id,"PLG595",6))){	//hyunjee.yoon@lge.com Need Change !
	...
	//if((strncmp(fw_product_id,"PLG595",6)!=0)&&(g_qem_check=='0')){
...
static int TD4100_SetLpwgMode(  TouchState newState, LpwgSetting  *pLpwgSetting)
{
	...
	if( (ts->currState != newState )|| force_set ) {
		...
		if((ts->currState == STATE_NORMAL) && (ime_stat == 1)){
			TOUCH_LOG("%s : IME STATE Set\n", __func__);
			synapitcs_change_ime_status(ds4_i2c_client, ime_stat);
		}
	}
...
static int TD4100_DoSelfDiagnosis_Lpwg( int* pRawStatus, char* buf, int bufSize, int* pDataLen)
{
	...
	if (1) {
...
#else
        lower_img = get_limit(F12_2DTxCount, F12_2DRxCount, *ts->client, defaultPanelSpec, "RT78FullRawCapUpperLimit", RT78FullRawCapUpper);
        upper_img = get_limit(F12_2DTxCount, F12_2DRxCount, *ts->client, defaultPanelSpec, "RT78FullRawCapLowerLimit", RT78FullRawCapLower);
#endif
       ...
       for(i=0;i<10;i++)
		...
		jitter = F54Test('o', 3, buf);
		msleep(30);
		//TOUCH_LOG("[%s] LPWG JITTER TEST Loop : %d\n",__func__,i);
		if(jitter <= 0){
			 *pRawStatus = TOUCH_FAIL;
			 TOUCH_LOG("[%s] LPWG JITTER TEST FAIL. Break Loop %d\n",__func__,i);
             break;
			}
       	}
        if(jitter > 0)
 	    TOUCH_LOG("[%s] LPWG JITTER TEST PASS.\n",__func__);
...
/*
static int TD4100_PowerOn(int isOn)
{
    //VDD(onoff)
   // delay();
    //VIO(onoff);
