...
#include <linux/input/unified_driver_4_k6p/lgtp_platform_api.h>
#include <linux/input/unified_driver_4_k6p/lgtp_model_config.h>
...
#define CAP_FILE_PATH_MINIOS				"/data/touch/touch_self_test.txt"
#define CAP_FILE_PATH_NORMAL				"/mnt/sdcard/touch_self_test.txt"
#else
...
static int write_log(char *filename, char *data)
{
	...
	if(filename == NULL){
		...
		if(boot_mode != BOOT_NORMAL){
			fd = sys_open(CAP_FILE_PATH_MINIOS, O_WRONLY|O_CREAT|O_APPEND, 0666);
			TOUCH_DBG("write log in /data/touch/touch_self_test.txt\n");
		}else{
			fd = sys_open(CAP_FILE_PATH_NORMAL, O_WRONLY|O_CREAT|O_APPEND, 0666);
