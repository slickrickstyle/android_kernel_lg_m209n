...
#define RAW_DATA_SUM_MAX 3
#define RAW_DATA_SUM_MIN 0
...
static int  MIT300_PrintData(struct i2c_client *client, char *buf, int* result, int type)
{
	...
	for (row = 0 ; row < MAX_ROW ; row++) {
		...
		for (col = MAX_COL-1 ; col >= 0 ; col--) {
			...
			} else if (type == ABS_SHOW){
#if defined(TOUCH_MODEL_M4)
				if (mit_data[row][col] == limit_lower) {
					ret += sprintf(buf + ret,"%5d ", mit_data[row][col]);
				} else {
					ret += sprintf(buf + ret,"!%4d ", mit_data[row][col]);
					rawdata_sum += mit_data[row][col];
					error_count++;
					//*result = TOUCH_FAIL;
				}
#else
...
#if defined(TOUCH_MODEL_M4)
	if (type == ABS_SHOW && rawdata_sum >= limit_upper){
		*result = TOUCH_FAIL;
	}
#endif
...
#if defined(TOUCH_MODEL_M4)
	if(type == ABS_SHOW)
		ret += sprintf(buf + ret,"MAX = %d,  MIN = %d  (MAX - MIN = %d), Spec rower : %d, upper : %d, Rawdata Sum : %d\n\n",max_data, min_data, max_data - min_data, limit_lower, limit_upper, rawdata_sum);
	else
#endif
