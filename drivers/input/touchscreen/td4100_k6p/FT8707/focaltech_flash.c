...
//TOUCH_LOG("Change to STDI2cValue,REG1 = 0x%x,REG2 = 0x%x,REG3 = 0x%x, iRet=%d\n", reg_val[0], reg_val[1], reg_val[2], iRet);
...
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (k= 0; k< 1/*FTS_UPGRADE_LOOP*/; k++)
	{
		...
		if (reg_val[0] != bt_ecc)
		{
			TOUCH_LOG( "[FTS]--k=%d, ecc error! fw_ecc=%02x flash_ecc=%02x\n",
						k,reg_val[0],
		...
		else
		{
			...
			TOUCH_LOG(KERN_WARNING "k=%d,checksum fw_ecc=%X flash_ecc=%X i2c_ecc \n",k,reg_val[0],bt_ecc);//,reg_val_i2c[0]);
			break;
		}
	}
...
bool Upgrade_ReadPram(struct i2c_client * client,unsigned int Addr, unsigned char * pData, unsigned short Datalen)
{
	...
	//if (iCommMode == HY_I2C_INTERFACE)
	{
		...
		if (ret < 0)
		{
			TOUCH_LOG("[FTS] failed Upgrade_ReadPram = %d \n", ret);
			ReCode = false;
			return ReCode;
		}
	...
	*/
	...
	if(FlashAddr == dw_lenth)
	{
		...
		for(i=0; i<dw_lenth; i++)
		{
			if(pCheckBuffer[i] != pbt_buf[i])
			{
				TOUCH_LOG("read out checksum error = No %d \n", i);
				if(pCheckBuffer)
					kfree(pCheckBuffer);
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	if(pbt_buf == NULL) {
		TOUCH_ERR("Fail to allocate memory pbt_buf\n");
		return TOUCH_FAIL;
	}
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client, u8 *fw_buf, int len, u8 *bootfw_buf, int boot_len)
{
	...
	*/
		...
		||(fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x06) ||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
	{
		...
		if ((fw_buf[fw_len - 8] ^ fw_buf[fw_len - 6]) == 0xFF
			&& (fw_buf[fw_len - 7] ^ fw_buf[fw_len - 5]) == 0xFF
			&& (fw_buf[fw_len - 3] ^ fw_buf[fw_len - 4]) == 0xFF)
		{
			/* FW upgrade */
			pbt_buf = fw_buf;
			/* call the upgrade function */
