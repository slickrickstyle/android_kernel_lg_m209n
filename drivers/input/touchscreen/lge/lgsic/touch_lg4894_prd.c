...
static void prd_read_rawdata(struct device *dev, u8 type)
{
	...
	lg4894_reg_read(dev, m2_raw_data_offset,
	...
	case DOZE1_BLU_JITTER_TEST:
		if (type == DOZE1_BLU_JITTER_TEST) {
			...
			backlightness = rt4832_lcd_backlight_get_level();
#endif
			touch_msleep(742);
#if defined(TARGET_MT6735_K6P)
...
#elif defined(CONFIG_MSM8909_K6P)
			rt4832_lcd_backlight_set_level(backlightness);
#endif
			touch_msleep(278);
		}
...
static void start_firmware(struct device *dev)
{
	...
	lg4894_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, (void *)&cmd,
			sizeof(u32));
