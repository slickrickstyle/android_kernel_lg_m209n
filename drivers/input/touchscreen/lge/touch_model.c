...
#define GPIO_TOUCH_RESET			GPIO62
#define GPIO_TOUCH_RESET_M_GPIO		GPIO_MODE_00

#define GPIO_TOUCH_INT				GPIO10
#define GPIO_TOUCH_INT_M_EINT		GPIO_MODE_00

#define TOUCH_IRQ_FLAGS (IRQF_TRIGGER_FALLING | IRQF_ONESHOT)
#endif
#endif
...
int touch_get_dts_base(struct touch_core_data *ts)
{
...
#if defined(CONFIG_LGE_TOUCH_CORE_MTK) && defined(TARGET_MT6735_K6P)
	ts->int_pin = GPIO_TOUCH_INT;
	ts->reset_pin = GPIO_TOUCH_RESET;
	ts->irqflags = TOUCH_IRQ_FLAGS;
#else
	...
	PROPERTY_U32(np, "hide_coordinate", ts->role.hide_coordinate);
