...
{
	static struct regulator* vdd_vio[2];
	...
	for (i = 0; i < 2; i++) {
		if (vdd_vio[i] == NULL) {
			if (client != NULL) {
				vdd_vio[i] = regulator_get(&client->dev, id);
			} else {
				vdd_vio[i] = regulator_get(NULL, id);
			}
			if (IS_ERR(vdd_vio[i])) {
				error = PTR_ERR(vdd_vio[i]);
				TOUCH_ERR("failed to get regulator ( error = %d )\n", error);
				return;
			}

			error = regulator_set_voltage(vdd_vio[i], min_uV, max_uV);
			if (error < 0) {
		...
		if (strstr(vdd_vio[i]->supply_name, id) != NULL || client == NULL) {
			if (isOn) {
				error = regulator_enable(vdd_vio[i]);

				if (error < 0) {
					TOUCH_ERR("failed to enable regulator ( error = %d )\n", error);
					return;
				}
				TOUCH_LOG("regulator_enable %s %d\n", vdd_vio[i]->supply_name, regulator_get_voltage(vdd_vio[i]));
				msleep(15);
			} else {
				error = regulator_disable(vdd_vio[i]);
				if (error < 0) {
					TOUCH_ERR("failed to enable regulator ( error = %d )\n", error);
					return;
				}
				TOUCH_LOG("regulator_disable %s %d\n", vdd_vio[i]->supply_name, regulator_get_voltage(vdd_vio[i]));
			}
			break;
		}
	}
...
{
	static struct regulator *vdd_vio[2];
	char *vdd_vio_name[2] = { vdd_id, vio_id };
	int uV[2] = { vdd_uV, vio_uV };
	struct i2c_client *client = Touch_Get_I2C_Handle();
	int i = 0, error = 0;

	TOUCH_LOG("[Touch] %s : TouchPowerPMIC isOn:%d\n", __func__,isOn);

	for( i=0; i<2; i++ ){
	if (vdd_vio[i] == NULL) {
        if(client!=NULL){
            vdd_vio[i] = regulator_get(&client->dev, vdd_vio_name[i]);
        }else{
            vdd_vio[i] = regulator_get(NULL, vdd_vio_name[i]);
        }
		if (IS_ERR(vdd_vio[i])) {
			error = PTR_ERR(vdd_vio[i]);
			TOUCH_ERR("failed to get regulator ( error = %d )\n", error);
		...
		error = regulator_set_voltage(vdd_vio[i], uV[i], uV[i]);
		if (error < 0) {
			TOUCH_ERR("failed to set regulator voltage ( error = %d )\n", error);
			return;
			}
