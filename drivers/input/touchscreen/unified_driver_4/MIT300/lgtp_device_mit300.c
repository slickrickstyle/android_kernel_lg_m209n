...
static const char defaultFirmware[] = "melfas/mit300/m209n/melfas_mip4.bin";
#endif
...
#if defined(TOUCH_MODEL_M1V) || defined (TOUCH_MODEL_K6P)
#if defined(TOUCH_MODEL_M1V_VZW)
static const char defaultFirmware[] = "melfas/mit300/m1v/L0M50P1_01_01.bin";
#else
static const char firstFirmware[] = "melfas/mit300/m1v/L0M50P1_01_09.bin";
static const char secondFirmware[] = "melfas/mit300/m1v/L0M50P2_01_00.bin";
u8 *defaultFirmware = NULL;
#endif
...
#if defined(TOUCH_MODEL_ME0)
static const char defaultFirmware[] = "melfas/mit300/me0/L0M50P1_00_13.img";
#endif

#if defined(TOUCH_MODEL_K6B)
static const char defaultFirmware[] = "melfas/mit300/k6b/L0M50P1_00_24.bin";
#endif
...
#endif
#if defined(TOUCH_MODEL_M1V) || defined(TOUCH_MODEL_M2)
extern int jitter_max_data;
extern int jitter_min_data;
...
{
#if defined(TOUCH_MODEL_M1V)
	rt4832_lcd_backlight_set_level(0);
#endif
#if defined(TOUCH_MODEL_M2)
...
{
#if defined(TOUCH_MODEL_M1V)
	rt4832_lcd_backlight_set_level(255);
#endif
#if defined(TOUCH_MODEL_M2)
...
#if defined(TOUCH_MODEL_M2) && defined(CONFIG_LGE_DISPLAY_HALLIC_FOLDER)
...
int mip_lpwg_config(struct i2c_client* client)
{
	...
	wbuf[13] = LPWG_DEBUG_STATUS;			// LPWG_FAIL_REASON
...
static ssize_t show_rawdata(TouchDriverData *pDriverData, char *buf)
{
	...
	if (!(pDriverData->lpwgSetting.lcdState | LPWG_DEBUG_STATUS)){
...
static ssize_t show_debug_mode(TouchDriverData *pDriverData, char *buf)
{
	int ret = 0;
	ret += sprintf(buf + ret,"Debug mode = %d\n", LPWG_DEBUG_STATUS);
	TOUCH_LOG("Debug mode = %d\n", LPWG_DEBUG_STATUS);
...
static ssize_t show_delta(TouchDriverData *pDriverData, char *buf)
{
	...
	mutex_lock(pMutexTouch);
	ret = MIT300_GetTestResult(client, buf, &deltaStatus, DELTA_SHOW);
	if (ret < 0) {
...
static ssize_t store_debug_mode(TouchDriverData *pDriverData, const char *buf, size_t count)
{
	int debug_mode = 0;
    if ( sscanf(buf, "%d", &debug_mode) != 1) {
		TOUCH_LOG("data parsing fail.\n");
		return -EINVAL;
	}
	ts->debug = debug_mode;
	TOUCH_LOG("Set Debug mode = %d\n", LPWG_DEBUG_STATUS);
	return count;
...
static LGE_TOUCH_ATTR(debug, S_IRUGO | S_IWUSR, show_debug_mode, store_debug_mode);
...
#if defined(TOUCH_MODEL_M1V)
#if !defined(TOUCH_MODEL_M1V_VZW)
	if (!TouchReadGpioMakerID()) {
			defaultFirmware = (char *)firstFirmware;
			TOUCH_LOG("MakerID is Low. P5 panel.\n");
	} else {
			defaultFirmware = (char *)secondFirmware;
			TOUCH_LOG("MakerID is High. P4 panel.\n");
	}
#endif
#endif
...
static int MIT300_InterruptHandler(TouchReadData *pData)
{
	...
	if ( (LPWG_DEBUG_STATUS ) == 0 && (ts->currState == STATE_KNOCK_ON_ONLY || ts->currState == STATE_KNOCK_ON_CODE)) {
	...
	if( packet_type == 0 )	/* Touch event */
	{
		for( i = 0 ; i < packet_size ; i += 6 )
		{
			...
			if( state )
			{
				if(ts->currState == STATE_KNOCK_ON_ONLY || ts->currState == STATE_KNOCK_ON_CODE){
					TOUCH_ERR("Unmatched IC-Driver State (index : %d)\n", index);
					return TOUCH_SUCCESS;
				}
				...
				if(cover_status){
					cover_x_temp ^= 1;
					pFingerData->x = (tmp[2] | ((tmp[1] & 0x0f) << 8)) + cover_x_temp;
				}
				else
#endif
...
static int MIT300_DoSelfDiagnosis(int* pRawStatus, int* pChannelStatus, char* pBuf, int bufSize, int* pDataLen)
{
	...
	*pDataLen = 0;
	...
	if (lge_get_factory_boot() == 1) {
		if(lge_get_mfts_mode())	sd_path = (char *) mfts_sd_path;
		else sd_path = (char *)	factory_sd_path;
	} else {
	...
	*pChannelStatus = TOUCH_SUCCESS;
	...
	// raw data check
	ret = MIT300_GetTestResult(client, pBuf, pRawStatus, ABS_SHOW);
	if( ret < 0 ) {
		TOUCH_ERR("failed to get raw data\n");
		memset(pBuf, 0, bufSize);
	...
	dataLen = sprintf(pBuf,"Jitter Max: %d,%d Jitter Average Max: %d,%d\n",jitterMax[0],jitterMax[1],jitterAverageMax[0],jitterAverageMax[1]);

#if defined(TOUCH_MODEL_M1V)
	rt4832_lcd_backlight_set_level(177);
#endif
#if defined(TOUCH_MODEL_M2)
    ...
    // open short check
    ret = MIT300_GetTestResult(client, pBuf+dataLen, pChannelStatus, OPENSHORT_SHOW);
    if (ret < 0) {
       TOUCH_ERR("failed to get open short data\n");
       memset(pBuf+dataLen, 0, bufSize);
       *pChannelStatus = TOUCH_FAIL;
    }
    MIT300_WriteFile(sd_path, pBuf+dataLen, 0);
    msleep(30);
    memset(pBuf+dataLen, 0, bufSize);

    // open short 2 check (MUX)
    ret = MIT300_GetTestResult(client, pBuf+dataLen, pChannelStatus, MUXSHORT_SHOW);
    if (ret < 0) {
