...
*******************************************************************************/
#define LGTP_MODULE "[FT8606_ex_fun]"
...
#define GTP_ESD_PROTECT 	0  // bringup lmh
/*******************************************************************************
...
static ssize_t fts_fwupdate_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	FT8606_Get_DefaultFWName(&app_name, &boot_name);
	i_ret = FT8606_RequestFirmware(app_name, boot_name, &app_fw, &boot_fw);
	if(i_ret == TOUCH_FAIL) {
...
//	i_ret = fts_ctpm_fw_upgrade_with_i_file(client);
	i_ret = fts_ctpm_fw_upgrade_with_i_file(client, (u8*)app_fw->data, app_fw->size, (u8*)boot_fw->data, boot_fw->size);
	...
	if (i_ret == 0) {
		...
		TOUCH_LOG("%s [FTS] upgrade to new version 0x%x\n", __func__, uc_host_fm_ver);
	} else {
