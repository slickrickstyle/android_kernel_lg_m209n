		...
		const char *buf, size_t count)
{
	...
	if (value == QUICKCOVER_CLOSE || value == QUICKCOVER_OPEN) {
		...
		touch_blocking_notifier_call(NOTIFY_QUICKCOVER_STATE,
					&ts->state.quick_cover);
		TOUCH_I("%s : %s(%d)\n", __func__,
			value ? "CLOSE" : "OPEN", value);
	} else {
		TOUCH_I("%s : Unknown %d\n", __func__, value);
...
static ssize_t show_fmradio_state(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int value = 0;
	int ret = 0;

	value = atomic_read(&ts->state.fmradio);
...
static ssize_t store_fmradio_state(struct device *dev,
		const char *buf, size_t count)
{
	int value = 0;
	struct touch_core_data *ts = to_touch_core(dev);
	...
	atomic_set(&ts->state.fmradio, value);

	TOUCH_I("%s : %s(%d)\n", __func__, value ? "ON" : "OFF", value);
...
static TOUCH_ATTR(fmradio_status, show_fmradio_state, store_fmradio_state);
...
static struct attribute *touch_attribute_list[] = {
	...
	&touch_attr_fmradio_status.attr,
	NULL,
};
