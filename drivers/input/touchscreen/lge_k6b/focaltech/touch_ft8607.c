...
static int ft8707_tci_control(struct device *dev, int type)
{
	...
	case TCI_CTRL_CONFIG_COMMON:
		...
		data = 0x21;
		ret |= ft8707_reg_write(dev, 0x89, &data, 1);	// Frame Rate for LPWG (33 Hz)
#endif
		if(atomic_read(&ts->state.fmradio)){ 
			// HW reqirement : weak radio status fm noise. 
			TOUCH_I("ft8707_lpwg_control fmradio mode.\n");
			data = 0x01;
			ret |= ft8707_reg_write(dev, 0x8C, &data, 1);	// fmradio mode
		}
...
//////////////////////////////////////////////
	...
	ret = lge_mdss_report_firmware_recovery();
	touch_msleep(300);
	
	if(ret != 0){
		TOUCH_I("lge_mdss_report_firmware_recovery didn't LCD reset : ret = %d\n", ret);
	}
