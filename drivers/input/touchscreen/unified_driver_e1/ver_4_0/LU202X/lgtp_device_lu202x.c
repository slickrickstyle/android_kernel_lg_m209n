...
static const char defaultFirmware[] = "leadingUI/E1_GF_OFFICIAL_01_11_160120.img";
...
{
	int lcdState = gDriverData->lpwgSetting.lcdState;
	int ret = 0;
	u8 retry_cnt = 3;
		...
		do {
			if (data == INCOMING_CALL_IDLE) {
				...
				if (lcdState) {
					...
					LU202X_LOG(BASE_INFO, "INCOMING_CALL_IDLE(Normal Mode)\n");
				} else {
				...
				if (lcdState) {
					ret = Lu202X_command_send(CMD_LU202x_CHANGEMODE,
							CMD_LU202x_DIAL_MODE, 0x0, NULL);
					LU202X_LOG(BASE_INFO, "%s(Dial Mode)\n", (data == INCOMING_CALL_RINGING) ?
