...
#if defined ( TOUCH_MODEL_LV1 )
static const char defaultFirmware[] = "melfas/mit200/lv1/fw.bin";
#endif
...
int mip_lpwg_config_knock_on(struct i2c_client* client, TouchState newState)
{
	u8 wbuf[32];
	...
	wbuf[15] = ((ts->lpwgSetting.isFirstTwoTapSame ? KNOCKON_DELAY : 0) >> 8) & 0xFF;  // LPWG_INTERTAP_DELAY (high byte)
        ...
        if (newState == STATE_KNOCK_CODE_ONLY)
        {
            wbuf[2] = 0;
        }
...
*/
	...
	switch( newState )
	{
		...
		case STATE_KNOCK_ON_ONLY :
			...
			//mip_lpwg_config_knock_on(client, newState);
		...
		//mip_lpwg_start(client);
			...
			mip_lpwg_start(client);
			break;
                case STATE_KNOCK_CODE_ONLY:
                        TOUCH_LOG("case STATE_KNOCK_CODE_ONLY\n");
                        if (ts->currState == STATE_OFF ){
                            TOUCH_LOG("No STATE_KNOCK_CODE_ONLY setting!!\n");
                        }else{
                            if(pDriverData->lpwg_debug_enable == 1){
                                TOUCH_LOG("[%s] lpwg_debug_enable status = %d\n",__func__,pDriverData->lpwg_debug_enable);
				mip_lpwg_debug_enable(client);
...
static LGE_TOUCH_ATTR(rawdata, S_IRUGO | S_IWUSR, show_lpwg_raw_data, NULL);
...
static int MIT200_SetLpwgMode(struct i2c_client *client, TouchState newState, LpwgSetting  *pLpwgSetting)
{
	...
	switch( newState )
	{
		...
		case STATE_KNOCK_ON_CODE:
			...
			break;
                case STATE_KNOCK_CODE_ONLY:
                        TOUCH_LOG("device was set to KNOCK_CODE_ONLY\n");
                        break;
		default:
...
void MIT200_mcu_sleep_control(struct i2c_client *client)
{
	...
	//gp93 high in normal
	if (pDriverData->lpwgSetting.proximityState == 0) { /*sensor near*/
