 ...
 *
 *
 * mit200_fw_upgrade.c : Firmware update functions for MIT200
 *
 */
...
static int mip_isc_read_status(struct melfas_ts_data *ts)
{
	...
	do {
		if(Mit200_proc4_I2C_Read(client, cmd, 6, &result, 1)){
			TOUCH_ERR("Mit200_proc4_I2C_Read failed\n");
			return -1;
		}
...
* Command : Erase Mass
*/
static int mip_isc_erase_mass(struct melfas_ts_data *ts)
{
	u8 write_buf[6] = ISC_CMD_ERASE_MASS;

	TOUCH_LOG("%s [START]\n", __func__);

	if (Mit200_proc4_I2C_Write(ts->client, write_buf, sizeof(write_buf))) {
		TOUCH_ERR("mip_isc_erase_mass failed\n");
		goto ERROR;
	}
...
* Command : Read Page
*/
static int mip_isc_read_page(struct melfas_ts_data *ts, int offset, u8 *data, int length)
{
	...
	if (Mit200_proc4_I2C_Read(ts->client, write_buf, sizeof(write_buf), data, length)) {
		TOUCH_ERR("Mit200_proc4_I2C_Write failed\n");
		goto ERROR;
	}
	//TOUCH_LOG("%s [DONE] - Offset [0x%04X]\n", __func__, offset);
...
* Command : Enter ISC
*/
//Disable this function in MIT200
#if 0
static int mip_isc_enter(struct melfas_ts_data *ts)
{
	u8 write_buf[8] = ISC_CMD_ENTER;

	if(Mit200_proc4_I2C_Write(ts->client, write_buf, 8)){
		TOUCH_ERR("Mit200_proc4_I2C_Write failed\n");
		goto ERROR;
	}
...
* Command : Write Page
*/
static int mip_isc_write_page(struct melfas_ts_data *ts, int offset, const u8 *data, int length)
{
	struct i2c_client *client = ts->client;
	u8 write_buf[6 + ISC_PAGE_SIZE] = ISC_CMD_WRITE_PAGE;

	if (length > ISC_PAGE_SIZE) {
		TOUCH_ERR("[ERROR] page length overflow\n");
		goto ERROR;
	}
	...
	if (Mit200_proc4_I2C_Write(client, write_buf, (length + 6))) {
		TOUCH_ERR("mip_isc_write_page failed\n");
		goto ERROR;
	}
	...
	//TOUCH_LOG("%s [DONE] - Offset[0x%04X] Length[%d]\n", __func__, offset, length);
	return 0;
...
static int mip_isc_exit(struct melfas_ts_data *ts)
{
	...
	if(Mit200_proc4_I2C_Write(ts->client, write_buf, 6)){
		TOUCH_ERR("Mit200_proc4_I2C_Write failed\n");
		goto ERROR;
	}
...
int mip_get_fw_version(struct melfas_ts_data *ts, u8 *ver_buf)
{
	...
	if(Mit200_proc4_I2C_Read(ts->client, wbuf, 2, rbuf, 8)){
		goto ERROR;
	};
...
int mip_flash_fw(struct melfas_ts_data *ts, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
	...
	print_hex_dump(KERN_ERR, LGTP_MODULE " Bin Info : ", DUMP_PREFIX_OFFSET, 16, 1, bin_info, tail_size, false);
#endif
	...
	//Enter ISC mode
	/*Disable this section in MIT200 protocol4 reference code kangmin1.lee@lge.com*/
	#if 0
	...
	//Set start offset
	offset_start = 0;
	TOUCH_LOG("Start offset[0x%04X]\n", offset_start);

	//Erase
	TOUCH_LOG("Erase Mass start\n");
	ret = mip_isc_erase_mass(ts);
	if (ret != 0) {
		TOUCH_ERR("[ERROR] mip_isc_erase_mass\n");
		ret = fw_err_download;
		goto ERROR;
	}

	//Write & Verify
	TOUCH_LOG("Write & Verify\n");
	offset = bin_size - ISC_PAGE_SIZE;
	while(offset >= offset_start){
		//Write page
		if (mip_isc_write_page(ts, offset, &bin_data[offset], ISC_PAGE_SIZE)){
			TOUCH_ERR("mip_isc_write_page : offset[0x%04X]\n", offset);
			ret = fw_err_download;
			goto ERROR;
		}
		//TOUCH_LOG("mip_isc_write_page : offset[0x%04X]\n", offset);

		//Verify
		if (mip_isc_read_page(ts, offset, rbuf, ISC_PAGE_SIZE)) {
			TOUCH_ERR("[ERROR] mip_isc_read_page : offset[0x%04X]\n", offset);
			ret = fw_err_download;
			goto ERROR;
		}
		//TOUCH_LOG("mip_isc_read_page : offset[0x%04X]\n", offset);

#if MIP_FW_UPDATE_DEBUG
		print_hex_dump(KERN_ERR, LGTP_MODULE " F/W File : ", DUMP_PREFIX_OFFSET, 16, 1, &bin_data[offset], ISC_PAGE_SIZE, false);
		print_hex_dump(KERN_ERR, LGTP_MODULE " F/W Chip : ", DUMP_PREFIX_OFFSET, 16, 1, rbuf, ISC_PAGE_SIZE, false);
#endif

		if(memcmp(rbuf, &bin_data[offset], ISC_PAGE_SIZE)){
			TOUCH_ERR("[ERROR] Verify failed : offset[0x%04X]\n", offset);
			ret = fw_err_download;
			goto ERROR;
		}
