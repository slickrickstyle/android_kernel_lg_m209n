...
static void SetDriverState(TouchDriverData *pDriverData, TouchState newState )
{
	...
	if( newState == STATE_BOOT ) {
        ...
        } else if ( newState == STATE_KNOCK_CODE_ONLY ) {
		TOUCH_LOG("STATE = KNOCK_CODE_ONLY\n");
	} else {
...
static void WqTouchIrqHandler(struct work_struct *work_irq)
{
	...
	{
		if( pDriverData->currState == STATE_KNOCK_ON_CODE || pDriverData->currState == STATE_KNOCK_CODE_ONLY)
		{
...
#if !defined ( TOUCH_MODEL_C70 ) && !defined ( TOUCH_MODEL_C90NAS ) && !defined ( TOUCH_MODEL_P1B ) && \
!defined ( TOUCH_MODEL_P1C ) && !defined ( TOUCH_MODEL_M209 ) && !defined ( TOUCH_MODEL_YG ) && \
!defined ( TOUCH_MODEL_M1 ) && !defined ( TOUCH_MODEL_LV1 )/* TBD : temporally block ( C70 is not ready ) */
	...
	else
	{
		...
		else
		{
			if( pLpwgSetting->mode == 0 ) {
				...
				TOUCH_LOG("next = STATE_KNOCK_CODE_ONLY\n");
				nextState = STATE_KNOCK_CODE_ONLY;
			} else {
...
static ssize_t store_lpwg_notify(struct i2c_client *client, const char *buf, size_t count)
{
	...
	if( ( nextState != STATE_UNKNOWN ) && ( pDriverData->currState != nextState ) )
	{
		...
		#if defined ( TOUCH_DEVICE_S3320 ) || defined ( TOUCH_DEVICE_MIT200 ) || defined ( TOUCH_DEVICE_MIT300 ) || defined( TOUCH_DEVICE_MIT200_PROC4 )
...
static ssize_t store_ic_rw(struct i2c_client *client, const char *buf, size_t count)
{
	...
	sscanf(buf, "%29s %x %x %d", cmd, &reg11, &reg21, &data);
...
static void touch_late_resume(struct early_suspend *h)
{
	...
	else /* NORMAL BOOT */
	{
		...
		#if defined ( TOUCH_DEVICE_MIT200 ) || defined ( TOUCH_DEVICE_MIT300 ) || defined( TOUCH_DEVICE_MIT200_PROC4 )
		queue_delayed_work(touch_wq, &pDriverData->work_init, 0);
		#endif
	}
...
#if !defined ( TOUCH_MODEL_M1 ) && !defined ( TOUCH_MODEL_LV1 )
	static int isEarlySuspend = TOUCH_FALSE;
#endif
	int *blank = NULL;
	#if defined ( TOUCH_DEVICE_S3320 ) && !defined ( TOUCH_MODEL_M1 ) && !defined (TOUCH_MODEL_LV1)
...
#endif
#if defined ( TOUCH_DEVICE_S3320 ) && !defined ( TOUCH_MODEL_M1 )
	/* WORK_AROUND_1 : skip first set of suspend and resume pair to avoid black screen on C70(S3320) */
	if( pDriverData->bootMode == BOOT_OFF_CHARGING )
	{
		if( skipCount > 0 ) {
...
!defined ( TOUCH_MODEL_M1 ) && !defined ( TOUCH_MODEL_LV1 )/* TBD : temporally block ( C70 is not ready ) */
...
#endif
	{
		if(event == FB_EARLY_EVENT_BLANK)
		{
			...
			{
				#if defined ( TOUCH_DEVICE_S3320 ) || defined ( TOUCH_DEVICE_MIT300 ) || defined ( TOUCH_DEVICE_MIT200 ) || defined( TOUCH_DEVICE_MIT200_PROC4 )
				if (lge_get_factory_boot()){
