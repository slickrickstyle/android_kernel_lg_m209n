...
static void TouchIrqEventHandler(TouchDriverData *pDriverData)
{
	...
	case DATA_HOVER:
        ...
        break;
	default:
		break; // fix me, temporal block because of first interrupt after knock on.
...
#if defined (TOUCH_DEVICE_SN280H)
		/* Re-setting LPWG mode  */
		if (pDriverData->nextState == STATE_KNOCK_ON_ONLY || pDriverData->nextState == STATE_KNOCK_ON_CODE) {
			TOUCH_LOG("Retry LPWG Setting\n");
			pDeviceControlFunc->SetLpwgMode(pDriverData->nextState, &pDriverData->lpwgSetting);
		}
#endif
	}
...
#if defined (TOUCH_DEVICE_FT3407)
        TOUCH_LOG("Power Off on BOOT_OFF_CHARGING\n");
        pDeviceControlFunc->Power(0);
        goto exit_device_init_fail;
#endif
	} else {
