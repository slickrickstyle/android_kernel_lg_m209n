...
int TouchGetModuleIndex(void)
{
    ...
    #if defined(TOUCH_MODEL_LION_3G) || defined (TOUCH_MODEL_E1)
    index = touch_module;
    #endif
...
int TouchGetBootMode(void)
{
	...
	#if defined(TOUCH_PLATFORM_MSM8916) || defined(TOUCH_PLATFORM_MSM8939) || defined (TOUCH_PLATFORM_MSM8909)
	{
...
int TouchInitializeGpio(void)
{
	...
	#if defined(TOUCH_PLATFORM_QCT)
	{
	...
	#endif
	#if defined(TOUCH_MODEL_ME0) || defined( TOUCH_MODEL_LV0 )
		...
		gpio_direction_output(TOUCH_GPIO_MAKER_ID, 0);
	#endif
	}
...
{
	int gpioState = 1; /* default value is "ONE" */

	#if defined(TOUCH_PLATFORM_QCT)
		gpioState = gpio_get_value(TOUCH_GPIO_MAKER_ID);
	#elif defined(TOUCH_PLATFORM_MTK)
