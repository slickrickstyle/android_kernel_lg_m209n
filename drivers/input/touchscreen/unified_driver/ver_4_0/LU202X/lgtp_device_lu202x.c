...
/* LPWG SD File Path */
#define LPWG_SD_FILE_PATH "/mnt/sdcard/lpwg_touch_self_test.txt"
...
static const char defaultFirmware[] = "leadingUI/E1_GF_TEST_00_A0_151013.img";
#elif defined ( TOUCH_MODEL_CLING )
static const char defaultFirmware[] = "leadingUI/Cling_LU2020_TEST_00_02_151210.img";
#else
...
static ssize_t show_hover_on_test(TouchDriverData *pDriverData, char *buf)
{
	u8 buffer[5]={CMD_LU202x_CHANGEMODE, 0, 0, 0, 0};
	int ret = 0;
	LU202X_LOG(BASE_INFO, "HOVER ON");

	WakeUpLu202x(TOGGLEWAKEUP);
	buffer[1] = 0x12;
	buffer[3] = 0x0;
	Lu20xx_I2C_Write(LU202x_MODE_ADDR, buffer, sizeof(buffer));
	SleepInLu202x();
...
static ssize_t show_hover_off_test(TouchDriverData *pDriverData, char *buf)
{
	int ret = 0;
	LU202X_LOG(BASE_INFO, "HOVER OFF");
	TouchSetGpioReset(0);
	msleep(10);
	TouchSetGpioReset(1);
	msleep(200);
...
int Lu202x_InterruptHandler(TouchReadData *pData)
{
	...
	switch (touch_data.EventType) {
		...
		LU202X_LOG(BASE_INFO, "[HOVERING NEAR] Event Type = %d\n", touch_data.EventType);
		/*To Do*/
		SleepInLu202x();
		break;

        case EVENT_HOVERING_FAR :
		LU202X_LOG(BASE_INFO, "[HOVERING FAR] Event Type = %d\n", touch_data.EventType);
		/*To Do*/
		SleepInLu202x();
		break;
...
int Lu202x_SetLpwgMode(TouchState newState, LpwgSetting  *pLpwgSetting)
{
	...
	for (i = 0; i < 3; i++) {
		if (newState == STATE_NORMAL) {
			...
			result = Lu202X_command_send(CMD_LU202x_CHANGEMODE, CMD_LU202x_KNOCK_ON_ONLY, 0x0, NULL);
			LU202X_LOG(BASE_INFO, "LPWG Mode Changed to KNOCK_ON_ONLY\n");
		} else if (newState == STATE_KNOCK_ON_CODE) {
			result = Lu202X_command_send(CMD_LU202x_CHANGEMODE, CMD_LU202x_KNOCK_ON_CODE, 0x0, NULL);
			LU202X_LOG(BASE_INFO, "LPWG Mode Changed to KNOCK_ON_CODE.\n");
		} else if (newState == STATE_OFF) {
			result = Lu202X_command_send(CMD_LU202x_CHANGEMODE, CMD_LU202x_PDN, 0x0, NULL);
...
static int LU202x_Lpwg_ChCap_Test(char* pBuf, int* lpwgStatus, int* pDataLen)
{
	...
	*lpwgStatus = TOUCH_SUCCESS;
	...
	for (i = 0; i < MAX_CHANNEL; i++) {
		/* compare test limits with readed rawcap */
		if (COMB16(Raw_Cap_Value, i) <= Lu202x_LPWG_LowerLimit[i] || \
			COMB16(Raw_Cap_Value, i) >= Lu202x_LPWG_UpperLimit[i]) {
			dataLen += sprintf(pBuf + dataLen, "!!!Raw data spec over and lower ch:%d[%d]\n", \
					i, COMB16(Raw_Cap_Value, i));
...
void Lu202x_file_write ( char *fname, char *data, int fsize )
{
	...
	else if(!strcmp(fname, LPWG_SD_FILE_PATH)) {
		fd = sys_open(fname, O_WRONLY|O_CREAT|O_APPEND|O_SYNC, 0644);
	}
...
int Lu202x_DoSelfDiagnosis_Lpwg(int *lpwgStatus, char *pBuf, int bufSize, int *pDataLen)
{
	...
	Lu202x_file_write(LPWG_SD_FILE_PATH, pBuf, strlen(pBuf));
	memset(pBuf, 0x00, bufSize);
	dataLen = 0;
