...
#define NVBIN_FILE_FCC     "wlan/prima/WCNSS_qcom_wlan_nv_boot_FCC.bin"
...
// [BEGIN][LGE_WIFI][neo-wifi@lge.com] 2016-06-02, Select specific NV file in one binary image for K6B project
#ifdef CONFIG_MACH_MSM8909_K6B_GLOBAL_COM
    {
        extern char *saved_command_line;
        char *pchModel = NULL;
        char *pchCmd= NULL;
        char *pchNVFileName = NULL;
        int iFCCModel = 1;  // Set default NV Path with "wlan/prima/WCNSS_qcom_wlan_nv_boot_FCC.bin"

        pchCmd = strstr(saved_command_line, "model.name=") + strlen("model.name=");
        if (pchCmd != NULL) {
            pchModel = strsep(&pchCmd, " ");
            if (pchModel != NULL) {
                pr_err("wcnss: Model Name is %s \n", pchModel);
                if (!strcmp(pchModel, "LG-K200DS")) {
                    iFCCModel = 0;
                }
            }
        }
        if (iFCCModel == 0) {
            pchNVFileName = NVBIN_FILE;
        } else {
            pchNVFileName = NVBIN_FILE_FCC;
        }
        pr_err("wcnss: NV path is %s \n", pchNVFileName);
        ret = request_firmware(&nv, pchNVFileName, dev);
    }
#else
    ret = request_firmware(&nv, NVBIN_FILE, dev);
#endif
// [END][LGE_WIFI][neo-wifi@lge.com] 2016-06-02, Select specific NV file in one binary image for K6B project
