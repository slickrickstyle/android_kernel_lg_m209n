...
static int lm3697_write_reg(struct i2c_client *client, unsigned char reg, unsigned char val)
{
	...
	if (err < 0)
		...
		pr_info("[hyuns] i2c write error reg: %d, val: %d\n", buf[0], buf[1]  );
...
static void lm3697_set_main_current_level(struct i2c_client *client, int level)
{
	...
	pr_err("[hyuns] %s: Brightness Level = %d\n", __func__, level);
		...
		if (flag_mode == LGE_PANEL_MODE_U2){
			...
			if (dev->blmap) {
				if (level < dev->blmap_size) {
					...
					dev_warn(&client->dev, "[hyuns] invalid index %d:%d\n", dev->blmap_size, level);
			} else {
				cal_value = level;
				lm3697_write_reg(client, 0x21, cal_value);
				//lm3697_write_reg(client, 0x23, cal_value);
				pr_info("[hyuns] %s cal_value blmap else %d\n", __func__, cal_value);
			}
#if IS_ENABLED(CONFIG_LGE_DISPLAY_AOD_SUPPORT)
		}
#endif
	} else {
		pr_info("[hyuns] %s backlight off level : %d\n", __func__, level);
		lm3697_write_reg(client, 0x24, 0x00);
	}
	mutex_unlock(&dev->bl_mutex);

	pr_info("[hyuns] %s : level=%d, cal_value=%d \n", __func__, level, cal_value);
}
...
static int bl_set_intensity(struct backlight_device *bd)
{
	...
	if(bd->props.brightness == cur_main_lcd_level){
		pr_info("[hyuns] %s level is already set. skip it\n", __func__);
		return 0;
	}

	pr_info("[hyuns] %s bd->props.brightness : %d     cur_main_lcd_level : %d \n", __func__, bd->props.brightness, cur_main_lcd_level);
		...
		struct backlight_platform_data *pdata)
{
	...
	if (pdata->blmap_size) {
		...
		pr_err("[hyuns] %s:%d, read backlight map OK\n",__func__, __LINE__);
