...
#if defined(CONFIG_BACKLIGHT_SM5306)
extern void sm5306_lcd_backlight_set_level(int level);
#endif

#if defined(CONFIG_BACKLIGHT_RT9393)
//extern void rt9393_lcd_backlight_set_level(int level);
#endif
							...
							u32 bl_level)
{
	...
	case BL_DCS_CMD:
		...
		break;
#if defined(CONFIG_BACKLIGHT_LM3530) || defined(CONFIG_BACKLIGHT_LM3632) || defined(CONFIG_BACKLIGHT_LM3697) || defined(CONFIG_BACKLIGHT_RT4832) || defined(CONFIG_BACKLIGHT_RT8542) || defined(CONFIG_BACKLIGHT_RT9393) || defined(CONFIG_BACKLIGHT_SM5306)
	case BL_OTHERS:
#if defined(CONFIG_BACKLIGHT_LM3530)
		...
		break;
#elif defined(CONFIG_BACKLIGHT_SM5306)
		sm5306_lcd_backlight_set_level(bl_level);
		break;
#elif defined(CONFIG_BACKLIGHT_RT9393)
		//rt9393_lcd_backlight_set_level(bl_level);

		if (gpio_is_valid(35)) {
			gpio_direction_output(35, 1);
			gpio_set_value_cansleep(35, 1);
			mdelay(1);
			pr_info("%s: RT9393 Backlight gpio = 35\n", __func__);
		}
		else
			pr_err("%s: RT9393 Backlight gpio is not valid !!\n", __func__);
...
#if defined(CONFIG_BACKLIGHT_RT9393)
int bl_pump(int count)
{
	static int i = 0;
	int cnt = i+count;
	dprintf(INOF, "RT9393 bl_pump_kernel !!\n");
	...
	dprintf(INFO, "cnt = %d, RT9393 bl_pump brightness = %d/32 kernel\n", i, 32-(i%32));
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	lcd_maker_id = (911+16);//= of_get_named_gpio(np,"lge,lcd_maker_id",0);
	if( !gpio_is_valid(lcd_maker_id) )
		pr_err("lcd_maker_id is not specified\n");
#endif
...
#if defined( CONFIG_LGD_DB7400_HD_5INCH_VIDEO_PANEL )
	if( gpio_get_value(lcd_maker_id) ) {
		pr_err("_P4_Panel\n");
		mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
			"qcom,mdss-dsi-on-command_p4", "qcom,mdss-dsi-on-command-state");
	}
	else {
		pr_err("_P5_Panel\n");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
...
#elif defined(CONFIG_LGD_SSD2068_FWVGA_VIDEO_PANEL)
	if(lge_get_mfts_mode()){
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
		"qcom,mdss-dsi-on-toggle-command", "qcom,mdss-dsi-on-command-state");
	pr_info("[Display] parsing Toggle cmd\n");
	} else {
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
		"qcom,mdss-dsi-on-command", "qcom,mdss-dsi-on-command-state");
	pr_info("[Display] parsing Alwayson cmd\n");
	}
#else
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
		"qcom,mdss-dsi-on-command", "qcom,mdss-dsi-on-command-state");
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->off_cmds_extra,
		"qcom,mdss-dsi-off-command2", "qcom,mdss-dsi-on-command-state");
#else
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->off_cmds,
