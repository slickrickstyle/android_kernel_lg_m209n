	...
	unsigned int cmd, void *arg)
{
...
#if defined(CONFIG_MSM8909_E1Q)
		if(((struct msm_vfe_axi_stream_cfg_cmd *)arg)->cmd == START_STREAM)
			start_setting++;
		else
			start_setting--;
		if(start_setting<0)
	...
	case 0xFFFF:
		mutex_lock(&vfe_dev->core_mutex);
		if(start_setting)
			test_set_sof_group_off();
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case 0xEEEE:
		mutex_lock(&vfe_dev->core_mutex);
		if(start_setting)
			test_set_eof_group_off();
		mutex_unlock(&vfe_dev->core_mutex);
		break;
#endif
	default:
...
int msm_isp_open_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct vfe_device *vfe_dev = v4l2_get_subdevdata(sd);
	long rc = 0;
	uint32_t irq1_mask, irq1_status;
	ISP_DBG("%s\n", __func__);
