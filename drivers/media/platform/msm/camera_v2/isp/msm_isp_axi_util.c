...
void msm_eof_notify(struct vfe_device *vfe_dev, enum msm_vfe_input_src frame_src, struct msm_isp_timestamp *ts){
	...
	msm_isp_send_event(vfe_dev, 0x8000500, &event_data);
}
void msm_sof_notify(struct vfe_device *vfe_dev, enum msm_vfe_input_src frame_src, struct msm_isp_timestamp *ts){
	struct msm_isp_event_data event_data;
	...
	event_data.timestamp = ts->event_time;
	event_data.mono_timestamp = ts->buf_time;
	msm_isp_send_event(vfe_dev, 0x8000400, &event_data);
}
	...
	struct msm_vfe_axi_stream *stream_info)
{
	...
	for (i = 0; i < stream_info->num_planes; i++) {
		if (stream_info->state == START_PENDING ||
			...
			pr_err("__debug__: WM[%d] DISABLE, src = %d, max_width = %u\n",
					stream_info->wm[i], stream_info->stream_src, stream_info->max_width);
			vfe_dev->hw_info->vfe_ops.axi_ops.
