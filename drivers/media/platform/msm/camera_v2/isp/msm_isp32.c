...
void set_camera_res(int mode)
{
	recording_mode = mode;
}
EXPORT_SYMBOL_GPL(set_camera_res);
	...
	struct msm_isp_timestamp *ts)
{
	...
	if (irq_status0 & BIT(0)) {
		ISP_DBG("%s: SOF IRQ\n", __func__);
#if defined(CONFIG_MSM8909_E1Q)
		msm_sof_notify(vfe_dev, VFE_PIX_0,ts);
#endif
...
#if defined(CONFIG_MSM8909_E1Q)
	else if ((irq_status0 == 0x4002 || (irq_status0&&0x6) == 0x6) && recording_mode == 1) {
			msm_eof_notify(vfe_dev, VFE_PIX_0,ts);
		}
	else if (irq_status0 == 0x4002 && recording_mode == 0) {
		msm_eof_notify(vfe_dev, VFE_PIX_0,ts);
	}
#endif
}
	...
	uint32_t first_start, uint32_t blocking)
{
	...
	if (blocking) {
		rc = wait_for_completion_timeout(
			&vfe_dev->reset_complete, msecs_to_jiffies(500)); //50 -> 500
	}
