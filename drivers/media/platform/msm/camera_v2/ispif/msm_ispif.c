        ...
        struct msm_cam_clk_info *clk_info)
{
        ...
        for (i = 0; i < count; i++) {
                ...
                pr_err("%s: clock-name= %s, clk_rate = %ld clock-control = %s\n",
	...
	struct ispif_irq_status *out, enum msm_ispif_vfe_intf vfe_id)
{
	...
	// LGE_Change_S, Fix for TD ID 105734, 06-17-15, ranjith.k@lge.com
	if (!ispif || !ispif->sof_count) {
...
static int ispif_probe(struct platform_device *pdev)
{
	...
	if (rc < 0){
		pr_err("%s: msm_isp_get_regulator() failed", __func__);
		return -EFAULT;
	}
