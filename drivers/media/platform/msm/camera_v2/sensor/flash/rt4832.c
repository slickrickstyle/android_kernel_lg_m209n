...
unsigned char strobe_ctrl_rt4832 = 0;
...
//set vin monitor value
	  rc = rt4832_read_byte(rt4832_fl->rt4832, 0x08, &status);
	  pr_err("flash read 0x08 status : %d \n", status);
	  status &= 0xF8; //clear 0x08[2:0] to 0
	  status |= 0x07; //set vin monitor 3.2V
	  rc = rt4832_write_byte(rt4832_fl->rt4832, 0x08, status);
	  pr_err("flash set to 0x08 status : %d \n", status);
	  if (rc < 0) {
		pr_err("%s:%d write to 0x08 failed\n", __func__, __LINE__);
	}

//set vin monitor
	strobe_ctrl_rt4832 = 0;
	rc = rt4832_read_byte(rt4832_fl->rt4832, 0x09, &strobe_ctrl_rt4832); // 58
	if (rc < 0) {
		pr_err("%s:%d failed\n", __func__, __LINE__);
	}
	pr_err("flash low read 0x09 status : %d \n", strobe_ctrl_rt4832);

	strobe_ctrl_rt4832 &= 0xEF;
	strobe_ctrl_rt4832 |=0x01;
	rc = rt4832_write_byte(rt4832_fl->rt4832, 0x09, strobe_ctrl_rt4832);
	pr_err("flash set to 0x09 status : %d \n", strobe_ctrl_rt4832);
	if (rc < 0) {
	...
	status = 0;
	rc = rt4832_read_byte(rt4832_fl->rt4832, 0x0B, &status);
	pr_err("flash read 0x0B status : %d \n", status);
...
#elif defined(CONFIG_MSM8909_K6P) || defined(CONFIG_MSM8909_LV1)
...
//set vin monitor
	...
	pr_err("flash high read 0x09 status : %d \n", strobe_ctrl_rt4832);
