...
/* LGE_CHANGE_S, apply proper power setting, 2014-10-29, yt.jeon@lge.com */
	...
	rev_type = lge_get_board_revno();
	 ...
	 if ((rev_type >= HW_REV_A) && slave_info->power_setting_array.size_a){ //Rev A
		/* DOWN */
	...
	else if ((rev_type > HW_REV_EVB2) && slave_info->power_setting_array.size_0){ //Rev 0
		/* DOWN */
...
/* LGE_CHANGE_S, apply proper power setting, 2014-10-29, yt.jeon@lge.com */
	{
#if defined(CONFIG_LGE_PM_PCB_REV)
		if ((slave_info->power_setting_array.power_setting_a!= NULL ) && (rev_type >= HW_REV_A)){ //Rev A
		...
		else if ((slave_info->power_setting_array.power_setting_0 != NULL) && (rev_type > HW_REV_EVB2)){ //Rev 0
			if (copy_from_user(pu,
