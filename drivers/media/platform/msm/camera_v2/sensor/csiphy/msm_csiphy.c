	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	while (lane_mask & 0x1f) {
		...
		if(csiphy_dev->pdev->id == 0){	//main camera
			if(csiphy_dev->hw_version >= CSIPHY_VERSION_V30) {
				msm_camera_io_w(0xd, csiphybase + csiphy_dev->ctrl_reg->csiphy_reg.
				mipi_csiphy_lnn_cfg4_addr + 0x40*j);
			pr_info("main camera mipi termination time tuning to d\n");
			}
		}
		else {  //sub camera
			if(csiphy_dev->hw_version >= CSIPHY_VERSION_V30) {
...
#else
		if(csiphy_dev->hw_version >= CSIPHY_VERSION_V30) {
			msm_camera_io_w(0xFF, csiphybase + csiphy_dev->ctrl_reg->csiphy_reg.
			mipi_csiphy_lnn_cfg4_addr + 0x40*j);
			pr_info("main camera mipi termination time tuning to 0xFF\n");
		}
#endif
