...
static struct msm_sensor_ctrl_t *s_ctrl_259 = NULL;
static int AE_flag =0;
static uint32_t LINE_trigger=0;
static uint32_t store_line=333;
static int recording_mode=0;
static uint32_t trigger=0;
static int antibanding=1;
#endif
	...
	struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	pr_err("%s:%d slave addr 0x%x sensor reg 0x%x id 0x%x\n",
		__func__, __LINE__,
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(strncmp(sensor_name,"hi259",5)==0){
	rc = sensor_i2c_client->i2c_func_tbl->i2c_read(
		...
		CDBG("%s: byte read id: 0x%x \n", __func__, chipid);
		chipid = (chipid << 8) & 0xFF00;
	}
	else{
		rc = sensor_i2c_client->i2c_func_tbl->i2c_read(
			sensor_i2c_client, slave_info->sensor_id_reg_addr,
...
int msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	case CFG_GET_SENSOR_INIT_PARAMS:
		...
		break;
#if defined(CONFIG_MSM8909_E1Q)
	case CFG_AE_WRITE_I2C_ARRAY:{
		struct msm_camera_i2c_reg_setting conf_array;
		struct msm_camera_i2c_reg_array *reg_setting = NULL;
		if(strcmp("hi259",s_ctrl->sensordata->sensor_name)==0)
			if(AE_flag == 1)
				break;
		...
		if(strcmp("hi259",s_ctrl->sensordata->sensor_name)==0){
			for ( i=0; i<conf_array.size; i++){
				if (reg_setting[i].reg_addr==0x22)
					LINE_trigger = reg_setting[i].reg_data<<8;
				else if (reg_setting[i].reg_addr==0x23)
					LINE_trigger |= reg_setting[i].reg_data;
				else if(reg_setting[i].reg_addr==0x10){
					if(reg_setting[i].reg_data & 0x02)//recording
						recording_mode=1;
					else
						recording_mode=0;
					if(reg_setting[i].reg_data & 0x04)//antibanding
						antibanding = 1;
					else
						antibanding = 0;
					if(recording_mode){
						if(antibanding==1)//60hz
							trigger=666;
						else
							trigger=800;
					}
					...
					set_camera_res(recording_mode);

					if(store_line<trigger){//kernel n+1
						if(recording_mode)
						{
						reg_setting[i].reg_data=0x07;
						reg_setting[i+1].reg_data=0x07;
						reg_setting[i+3].reg_data=0x03;
						}
						else
						{
						reg_setting[i].reg_data=0x03;
						reg_setting[i+1].reg_data=0x03;
						reg_setting[i+3].reg_data=0x00;
						}
					}
					else {//kernel n+2
						if(recording_mode)
...
#if defined(CONFIG_MSM8909_E1Q)
static struct msm_camera_i2c_reg_array hynix_groupoff_reg_array[] = {
  { 0x1F, 0x00 },
};

void test_set_sof_group_off(void){
	int rc;
	struct msm_camera_i2c_reg_setting conf_array;

	if(s_ctrl_259 != NULL){
		if(AE_flag==1 && (store_line<trigger)&&recording_mode==0){
			conf_array.reg_setting = hynix_groupoff_reg_array;
			conf_array.addr_type = MSM_CAMERA_I2C_BYTE_ADDR;
			conf_array.data_type = MSM_CAMERA_I2C_BYTE_DATA;
			conf_array.delay =0;
			conf_array.size= 1;

			CDBG("%s groupoff register write store_line %X, trigger %X, recording_mode %d \n", __func__, store_line, trigger, recording_mode);
			rc = s_ctrl_259->sensor_i2c_client->i2c_func_tbl->
				i2c_write_table(s_ctrl_259->sensor_i2c_client,
				&conf_array);
		...
		CDBG("%s s_ctrl_259 error rc %d\n",__func__, rc);
	}

}
EXPORT_SYMBOL_GPL(test_set_sof_group_off);
void test_set_eof_group_off(void){
	int rc;
	struct msm_camera_i2c_reg_setting conf_array;

	if(s_ctrl_259 != NULL){
		if(AE_flag==1 && (store_line >=trigger || recording_mode==1)){
			conf_array.reg_setting = hynix_groupoff_reg_array;
			conf_array.addr_type = MSM_CAMERA_I2C_BYTE_ADDR;
			conf_array.data_type = MSM_CAMERA_I2C_BYTE_DATA;
			conf_array.delay =0;
...
EXPORT_SYMBOL_GPL(test_set_eof_group_off);
