...
#if defined(CONFIG_MACH_MSM8909_E1Q_VZW) || defined(CONFIG_MACH_MSM8909_CLING_VZW) || defined(CONFIG_MACH_MSM8909_M1V_VZW) || defined(CONFIG_MACH_MSM8909_M1V_LRA_US) || defined(CONFIG_MSM8909_LV1_VZW_US)
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CLK:
			...
			break;
		case SENSOR_GPIO:
#if defined(CONFIG_MSM8909_LV1_VZW_US)                  
              pr_err("CONFIG_MSM8909_LV1_VZW_US  by jmin \n");
...
#endif
#if defined(CONFIG_MACH_MSM8909_E1Q_VZW) || defined(CONFIG_MACH_MSM8909_CLING_VZW)
			if(pd->seq_val == SENSOR_GPIO_CUSTOM1){
				if(ctrl->vdig_ldo3 == NULL){
					ctrl->vdig_ldo3 = regulator_get(ctrl->dev, "rt5058-ldo3");
					if (IS_ERR(ctrl->vdig_ldo3)){
						pr_err("%s get failed rt5058-ldo3\n", __func__);
				...
				ret= regulator_disable(ctrl->vdig_ldo3);
				break;
			}
			else if(pd->seq_val == SENSOR_GPIO_CUSTOM2){
				if(ctrl->vdig_ldo2 == NULL){
					ctrl->vdig_ldo2 = regulator_get(ctrl->dev, "rt5058-ldo2");
					if (IS_ERR(ctrl->vdig_ldo2)){
						pr_err("%s get failed rt5058-ldo2\n", __func__);
				...
				ret = regulator_disable(ctrl->vdig_ldo2);
				break;
			}
			else if(pd->seq_val == SENSOR_GPIO_VANA){
				if(ctrl->vana_ldo1 == NULL){
					ctrl->vana_ldo1 = regulator_get(ctrl->dev, "rt5058-ldo1");
					if (IS_ERR(ctrl->vana_ldo1)){
						pr_err("%s get failed rt5058-ldo1\n", __func__);
				...
				ret = regulator_disable(ctrl->vana_ldo1);
				break;
			}
#endif
