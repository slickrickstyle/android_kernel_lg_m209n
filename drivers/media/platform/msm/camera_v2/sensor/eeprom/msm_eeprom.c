...
#if 1 // QCT Original,,.. msm8909 does not use CCI,,,, 
...
#else		
		if (emap[j].saddr.addr) {
			e_ctrl->i2c_client.client->addr =emap[j].saddr.addr;			
			pr_err("qcom,slave-addr = 0x%X\n",
			 ...
			 const struct i2c_device_id *id)
{
	...
	pr_err("[CHECK] [BEFORE] e_ctrl->is_supported: 0x%X", e_ctrl->is_supported);
	e_ctrl->is_supported |= msm_eeprom_checksum(e_ctrl);
	pr_err("[CHECK] [AFTER]  e_ctrl->is_supported: 0x%X", e_ctrl->is_supported);
	...
	msm_sd_register(&e_ctrl->msm_sd);
	e_ctrl->is_supported = (e_ctrl->is_supported << 1) | 1;
	pr_err("[CHECK] [FINAL]  e_ctrl->is_supported: 0x%X", e_ctrl->is_supported);
	pr_err("msm_eeprom_i2c_probe successfull");
#if defined(CONFIG_MACH_LGE)
