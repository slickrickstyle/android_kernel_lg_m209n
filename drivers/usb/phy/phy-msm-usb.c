...
#if defined(CONFIG_LGE_USB_G_MHL)
...
#if defined (CONFIG_LGE_TOUCH_CORE) || defined (CONFIG_LGE_TOUCH_CORE_K6B)
...
#ifdef CONFIG_LGE_PM_FLOATED_CHARGER
#define DCD_TIMEOUT_CNT_MIN 6 // do not edit this value.
#define DCD_TIMEOUT_CNT_MAX 30 // 750ms + (750ms + 2000ms)*(10+1) = 31000ms(31s)
#define CHG_DET_RETRY_DELAY msecs_to_jiffies(2000)
#endif
...
bool otg_cable_connected(struct msm_otg *motg)
{
	...
	rc = qpnp_vadc_read(motg->vadc_dev, LR_MUX2_BAT_ID, &result);
	if (rc < 0) {
...
void usbid_spdt_set(struct msm_otg *motg, int on)
{
	int ret;
	if (gpio_is_valid(motg->pdata->usb_id_switch)) {
		ret = gpio_request(motg->pdata->usb_id_switch, "SPDT_USB_ID");
		if (ret < 0)  {
				pr_debug("usb_id_switch request fail !! \n");
		}
		ret = gpio_direction_output(motg->pdata->usb_id_switch, on);
		if (ret < 0) {
				pr_debug("usb_id_switch direction output fail !! \n");
		}
	} else {
		pr_debug("usb_id_switch is not valid !!\n");
	}
}

int usbid_spdt_get(struct msm_otg *motg)
{
	return gpio_get_value(motg->pdata->usb_id_switch);
}
/*
int external_get_spdt_usbid (void)
{
	struct msm_otg *motg = the_msm_otg;
	int res;
	res = usbid_spdt_get(motg);
	return (res);
}

EXPORT_SYMBOL(external_get_spdt_usbid);
*/

void set_usb_id_to_batt_id(int on)
{
	int res;
	usbid_spdt_set(the_msm_otg, on);
	res = usbid_spdt_get(the_msm_otg);
	pr_info("[BATT_ID] USB_ID to BATT_ID: %d\n", res);

}
EXPORT_SYMBOL(set_usb_id_to_batt_id);
#endif
...
#if defined (CONFIG_LGE_PM_CABLE_DETECTION) && defined (CONFIG_SMB1351_USB_CHARGER)
static void lge_pm_read_usbid (struct msm_otg * motg)
{
	if (IS_ERR_OR_NULL(motg->vadc_dev)) {
		motg->vadc_dev = qpnp_get_vadc(motg->phy.dev, "phy");
		if (IS_ERR(motg->vadc_dev))
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_B_IDLE:
		...
		if ((!test_bit(ID, &motg->inputs) ||
			...
			(/*defined (CONFIG_LGE_PM_EXTERNAL_CHARGER_RT9536_SUPPORT) || */ \
			defined (CONFIG_SMB1351_USB_CHARGER))
					lge_pm_read_usbid(motg);
#endif
#ifdef CONFIG_LGE_PM
#ifdef CONFIG_LGE_USB_CHARGING_SPEC_VZW
...
#ifdef CONFIG_SMB1351_USB_CHARGER
/*
  *	To detect HVDCP Charger for Mini OS
  *	enum {
  *		HVDCP_NONE = 0,
  *		HVDCP_LEGACY_DCP,
  *		HVDCP_EVP_DYNAMIC,
...
*/
	case POWER_SUPPLY_PROP_HVDCP_TYPE:
		pr_info("[USB_PROPERTY] charger_type = %d\n", charger_type);
		if (charger_type == POWER_SUPPLY_TYPE_USB_HVDCP) {
			val->intval = 4; //HVDCP Charger Connected for MINIOS
		} else if (charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
			val->intval = 1; //DCP Charger Connected for MISIOS
		} else {
			val->intval = 0;
		}
		break;
...
#ifdef CONFIG_LGE_USB_ID_SPDT_SWITCH
	pdata->usb_id_switch =
			of_get_named_gpio(node, "usb-id-switch", 0);
	if (pdata->usb_id_switch < 0) {
		pr_debug("usb_id_switch is not available\n");
	}
#endif
	pdata->usb_id_gpio =
