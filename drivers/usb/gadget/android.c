...
static const bool cdrom_enable = true;
#else
static const bool cdrom_enable = false;
#endif
#ifdef CONFIG_LGE_USB_G_ANDROID
...
static void android_work(struct work_struct *data)
{
	...
	* E7IILTE models  : embedded type
	...
	if (uevent_envp == connected) {
		if (lge_pm_get_cable_type() == CABLE_56K &&
			...
			kernel_restart(NULL);
					...
					(lge_get_sbl_cable_type() != LT_CABLE_910K || !firstboot_check) &&
					!lge_get_laf_mode()) {
			usb_gadget_disconnect(cdev->gadget);
			usb_ep_dequeue(cdev->gadget->ep0, cdev->req);
	...
	if (uevent_envp == configured) {
		pr_info("[cable info] boot_mode:%d\n", lge_get_boot_mode());
		firstboot_check = 0;
	}
#endif
}
...
#ifdef CONFIG_LGE_USB_G_AUTORUN
	if ((dev->pdata && dev->pdata->cdrom) || cdrom_enable == true) {
