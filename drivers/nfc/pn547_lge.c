...
#ifdef CONFIG_PN5XX_MTK_PLATFORM
static irqreturn_t pn547_dev_irq_handler(void)
{
        ...
        const struct i2c_device_id *id)
{
    ...
    /* irq_gpio setup */
    NFC_LOG(__func__, "irq_gpio setup");
     ...
     */
    NFC_LOG1(__func__, "requesting IRQ", client->irq);
#ifdef CONFIG_PN5XX_MTK_PLATFORM
    ...
    if (node) {
          ...
          if (ret) {
              ...
              NFC_LOG1(__func__, "set EINT finished, nfc_irq", nfc_irq);
          }
    } else {
