...
#define TOUCH_CTRL_PATH "/sys/devices/virtual/input/lge_touch/hall_status"
#endif
...
struct pm8xxx_cradle {
	...
	spinlock_t lock;
	int state;
#if !defined CONFIG_LGE_DISPLAY_HALLIC_FOLDER
...
#if defined(CONFIG_MACH_MSM8909_M209N_TMO_US) || defined(CONFIG_MACH_MSM8909_M209N_MPCS_US) || defined(CONFIG_MACH_MSM8909_M209N_ATT_US) || defined(CONFIG_MACH_MSM8909_M209N_CRK_US)
extern void MIT300_Set_BootCoverMode(int status);
#endif

#if defined(CONFIG_TOUCHSCREEN_LGE_TD4100)
...
static void boot_cradle_det_func(void)
{
	...
	if (cradle->pdata->hallic_pouch_detect_pin) {
		cradle->pouch =
			!gpio_get_value(cradle->pdata->hallic_pouch_detect_pin);
	}
	...
	if (cradle->pdata->hallic_pen_detect_pin) {
		cradle->pen =
			!gpio_get_value(cradle->pdata->hallic_pen_detect_pin);
	}
	printk("%s : boot pen === > %d \n", __func__ , cradle->pen);
	...
	if (cradle->pdata->hallic_camera_detect_pin) {
		cradle->camera =
			!gpio_get_value(cradle->pdata->hallic_camera_detect_pin);
	}
...
#if defined(CONFIG_MACH_MSM8909_M209N_TMO_US) || defined(CONFIG_MACH_MSM8909_M209N_MPCS_US) || defined(CONFIG_MACH_MSM8909_M209N_ATT_US) || defined(CONFIG_MACH_MSM8909_M209N_CRK_US)
	smartcover_status =cradle->pouch;
    MIT300_Set_BootCoverMode(cradle->pouch);
#elif defined(CONFIG_MSM8909_K6B) || defined(CONFIG_MSM8909_K6P)
	smartcover_status =cradle->pouch;
#if defined(CONFIG_TOUCHSCREEN_LGE_TD4100)
	Touch_Quick_Cover_Closed = smartcover_status;
#endif
#endif
}
...
static void pm8xxx_pouch_work_func(struct work_struct *work)
{
	int state = 0;
	unsigned long flags;
#if !defined CONFIG_HALLIC_PEN && !defined CONFIG_LGE_DISPLAY_HALLIC_FOLDER
	int tmp_camera = 0;
#endif
	spin_lock_irqsave(&cradle->lock, flags);
	...
	if (cradle->state != state) {
		...
		if(lge_get_boot_mode()==LGE_BOOT_MODE_QEM_56K) {
			printk("[Touch] Write %d to %s\n",cradle->state, TOUCH_CTRL_PATH);
			write_to_file(TOUCH_CTRL_PATH, cradle->state);
		}
#endif
	}
	else {
...
#if defined(CONFIG_MACH_MSM8909_M209N_TMO_US) || defined(CONFIG_MACH_MSM8909_M209N_MPCS_US) || defined(CONFIG_MACH_MSM8909_M209N_ATT_US) || defined(CONFIG_MSM8909_K6B) || defined(CONFIG_MSM8909_K6P) || \
	defined(CONFIG_MACH_MSM8909_M209N_CRK_US)
	smartcover_status = !gpio_get_value(cradle->pdata->hallic_pouch_detect_pin);
	v = 1 + 1*smartcover_status;
	#if defined(CONFIG_TOUCHSCREEN_LGE_TD4100)
	if(!smartcover_status)//Cover Closed
		Touch_Quick_Cover_Closed = 1;
	else
		Touch_Quick_Cover_Closed = 0;
	#endif
...
#endif
	wake_lock_timeout(&cradle->wake_lock,
			msecs_to_jiffies(POUCH_DETECT_DELAY*v+5));
	queue_delayed_work(cradle_wq, &cradle_handle->pouch_work,
			msecs_to_jiffies(POUCH_DETECT_DELAY*v));
	return IRQ_HANDLED;
}
...
static irqreturn_t pm8xxx_pen_irq_handler(int irq, void *handle)
{
	...
	printk("pen irq!!!!\n");
	wake_lock_timeout(&cradle->wake_lock,
			msecs_to_jiffies(HALL_DETECT_DELAY+5));
	queue_delayed_work(cradle_wq, &cradle_handle->pen_work,
			msecs_to_jiffies(HALL_DETECT_DELAY));
	return IRQ_HANDLED;
}
#else
static irqreturn_t pm8xxx_camera_irq_handler(int irq, void *handle)
{
	...
	wake_lock_timeout(&cradle->wake_lock,
			msecs_to_jiffies(HALL_DETECT_DELAY+5));
	queue_delayed_work(cradle_wq, &cradle_handle->camera_work,
			msecs_to_jiffies(HALL_DETECT_DELAY));
	return IRQ_HANDLED;
}
#endif
...
static ssize_t
cradle_sensing_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int len;
	struct pm8xxx_cradle *cradle = dev_get_drvdata(dev);
...
#if !defined CONFIG_LGE_DISPLAY_HALLIC_FOLDER
#if defined CONFIG_HALLIC_PEN
static struct device_attribute cradle_pen_attr =
__ATTR(pen, S_IRUGO, cradle_pen_show, NULL);
#else
static struct device_attribute cradle_camera_attr =
__ATTR(camera, S_IRUGO, cradle_camera_show, NULL);
#endif
#endif
static struct device_attribute cradle_sensing_attr =
__ATTR(sensing, S_IRUGO, cradle_sensing_show, NULL);
static struct device_attribute cradle_pouch_attr =
...
#elif defined(CONFIG_BU52031NVX)
		...
		struct pm8xxx_cradle_platform_data *pdata)
{
	...
	*/
	if ((pdata->hallic_pouch_detect_pin =
				of_get_named_gpio_flags(np,
					"hallic-pouch-irq-gpio", 0, NULL)) > 0)
	{
		pdata->hallic_pouch_irq =
			gpio_to_irq(pdata->hallic_pouch_detect_pin);
	}
#if defined CONFIG_LGE_DISPLAY_HALLIC_FOLDER
	printk("[Hall IC] hallic_pouch_gpio: %d\n",
		pdata->hallic_pouch_detect_pin);
#else
#if defined CONFIG_HALLIC_PEN
	if ((pdata->hallic_pen_detect_pin =
				of_get_named_gpio_flags(np,
					"hallic-pen-irq-gpio", 0, NULL)) > 0)
	{
		pdata->hallic_pen_irq =
			gpio_to_irq(pdata->hallic_pen_detect_pin);
	}
	printk("[Hall IC] hallic_pouch_gpio: %d, hallic_pen_gpio: %d\n",
		pdata->hallic_pouch_detect_pin, pdata->hallic_pen_detect_pin);
#else
	if ((pdata->hallic_camera_detect_pin =
				of_get_named_gpio_flags(np,
					"hallic-camera-irq-gpio", 0, NULL)) > 0)
	{
		pdata->hallic_camera_irq =
			gpio_to_irq(pdata->hallic_camera_detect_pin);
	}
...
static int pm8xxx_cradle_probe(struct platform_device *pdev)
{
...
#else
	unsigned int hall_camera_gpio_irq = 0;
#endif
#endif
...
#if defined CONFIG_LGE_DISPLAY_HALLIC_FOLDER
#if defined(CONFIG_MACH_MSM8909_M209N_TMO_US) || defined(CONFIG_MACH_MSM8909_M209N_MPCS_US) || defined(CONFIG_MSM8909_K6B) || defined(CONFIG_MSM8909_K6P) || defined(CONFIG_MACH_MSM8909_M209N_ATT_US) || \
	defined(CONFIG_MACH_MSM8909_M209N_CRK_US)
	...
	if (cradle->pdata->hallic_pouch_detect_pin > 0) {
		...
		ret = request_irq(hall_pouch_gpio_irq,
				pm8xxx_pouch_irq_handler,
				pdata->irq_flags,
				HALL_IC_DEV_NAME,
				cradle);
		if (ret > 0) {
	...
	if (cradle->pdata->hallic_pen_detect_pin > 0) {
		...
		ret = request_irq(hall_pen_gpio_irq,
				pm8xxx_pen_irq_handler,
				pdata->irq_flags,
				HALL_IC_DEV_NAME,
				cradle);
		if (ret > 0) {
	...
	if (cradle->pdata->hallic_camera_detect_pin > 0) {
		...
		ret = request_irq(hall_camera_gpio_irq,
				pm8xxx_camera_irq_handler,
				pdata->irq_flags,
				HALL_IC_DEV_NAME,
				cradle);
		if (ret > 0) {
