			...
			struct device_attribute *devattr, char *buf)
{
	...
	if(hw_rev == HW_REV_B)
	{
		if(attr->index == 50 || attr->index == 60)
...
static bool is_910k_factory_cable(void)
{
...
#if defined (CONFIG_MSM8909_K6P)\
	||defined (CONFIG_MSM8909_K6B)
	if (is_910k_factory_cable()) {
		pr_info("[K6] Return get_temp for 910K Factory mode\n");
		return 0;
	}
#endif
