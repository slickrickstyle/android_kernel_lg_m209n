...
#define BATTERY_LOW_THR 2500000
static int get_battery_voltage(struct qpnp_bms_chip *chip, int *result_uv);
#endif
#ifdef CONFIG_LGE_USB_ID_SPDT_SWITCH
extern void set_usb_id_to_batt_id(int on);
#endif

#if defined (CONFIG_LGE_PM_EMBEDDED_BATTERY_INITIAL_SOC_COMP)
extern int set_charging_enable_for_bms(int enable);
#endif
...
static void check_recharge_condition(struct qpnp_bms_chip *chip)
{
	...
	pr_info("check_recharge_condition status = %d, eoc_reported = %d\n", \
			status, chip->eoc_reported);
...
static void check_eoc_condition(struct qpnp_bms_chip *chip)
{
	int rc;
	int status = get_battery_status(chip);
#ifndef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
	union power_supply_propval ret = {0,};
#endif
...
#ifdef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
#if defined (CONFIG_LGE_PM_BATTERY_CAPACITY_4100mAh)
#define SOC_RESCALING_FACTOR 93
#define EMPTY_SOC 156
#define ADJUST_RESCALING_FACTOR SOC_RESCALING_FACTOR*100 - EMPTY_SOC
#else
#define SOC_RESCALING_FACTOR 96
#endif
#if (defined (CONFIG_LGE_PM_BATTERY_CAPACITY_1940mAh) || \
     defined (CONFIG_LGE_PM_BATTERY_CAPACITY_2100mAh) || \
     defined (CONFIG_LGE_PM_BATTERY_CAPACITY_2125mAh) || \
     defined (CONFIG_LGE_PM_BATTERY_CAPACITY_2300mAh)|| \
     defined (CONFIG_LGE_PM_BATTERY_CAPACITY_2500mAh) || \
     defined (CONFIG_LGE_PM_BATTERY_CAPACITY_4100mAh))
...
#ifdef CONFIG_LGE_PM_BATTERY_CAPACITY_4100mAh
	recalc_soc = ((chip->last_soc * 100) - EMPTY_SOC) + 50;
	recalc_soc = DIV_ROUND_CLOSEST(recalc_soc*100, ADJUST_RESCALING_FACTOR);
#else
...
static int report_vm_bms_soc(struct qpnp_bms_chip *chip)
{
	...
	soc = bound_soc(soc);
	if ((soc != chip->last_soc) || (soc == 100)) {
		pr_info("check_eoc_condition soc=%d, last_soc=%d, charging=%d\n",
...
#if defined (CONFIG_LGE_PM_EMBEDDED_BATTERY_INITIAL_SOC_COMP)
#define ESTIMATE_DIFF_MV 150000 //150mV
int read_ocv_from_battery(struct qpnp_bms_chip *chip)
{
	int rc = 0;
	int vbat_ocv = 0;

	pr_info("[LGCC] Read a initial OCV from battery \n");
	mutex_lock(&chip->last_soc_mutex);
	rc = set_charging_enable_for_bms(0);
	if (rc) {
		pr_err("Unable to disable charging rc=%d\n", rc);
	}

	/*Delay 500ms to wait for charging disabled*/
	mdelay(500);

	vbat_ocv = estimate_ocv(chip);
	pr_info("[LGCC] Vbat check first = %d\n", vbat_ocv);
	mutex_unlock(&chip->last_soc_mutex);
	rc = set_charging_enable_for_bms(1);
	if (rc) {
...
#if defined (CONFIG_LGE_PM_EMBEDDED_BATTERY_INITIAL_SOC_COMP)
		if ( (is_charger_present(chip)) && chip->shutdown_soc_invalid ) {
			vbat_ocv = read_ocv_from_battery(chip);
			/* use vbat_ocv only if shutdown SOC is invalid */
			chip->last_ocv_uv = vbat_ocv;
			chip->calculated_soc = lookup_soc_ocv(chip, vbat_ocv, batt_temp);
			chip->last_soc = chip->calculated_soc;
			pr_info("Using vbat_ocv\n");
		} else {
...
#ifdef CONFIG_LGE_USB_ID_SPDT_SWITCH
#define LGC_4100MAH 400
#define TOCAD_PANA_4100MAH 401
#define ATL_4100MAH 402
static int read_lge_battery_id_adc(struct qpnp_bms_chip *chip)
{
	struct qpnp_vadc_result result;
	int batt_id_adc, rc = 0;

	set_usb_id_to_batt_id(1);
	rc = qpnp_vadc_read(chip->vadc_dev, LR_MUX2_BAT_ID, &result);
	batt_id_adc = result.physical;
	pr_info("[BATT_ID] Read a Batt ID ADC = %d\n",batt_id_adc);
	set_usb_id_to_batt_id(0);
	if (batt_id_adc > 900000) {
		pr_info("[BATT_ID] ATL battery upload\n");
		return ATL_4100MAH;
	} else if (batt_id_adc > 600000) {
		pr_info("[BATT_ID] TOCAD PANA battery upload\n");
		return TOCAD_PANA_4100MAH;	
	} else {
		pr_info("[BATT_ID] LGC battery upload\n");
		return LGC_4100MAH;
	}
}
#endif
...
#ifdef CONFIG_LGE_USB_ID_SPDT_SWITCH
	return read_lge_battery_id_adc(chip);
#else
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	default :
		...
		pr_info("[BATTERY PROFILE] No battery ID matching\n"\
				...
				"LGChem_2125mAh for id(%lld)\n", battery_id);
		break;
#elif defined(CONFIG_LGE_PM_BATTERY_CAPACITY_2500mAh)
	case BATT_ID_SW3800_VC0 :
	case BATT_ID_ISL6296A_L :
		battery_profile = "lgc,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"LGChem_2500mAh for id(%lld)\n", battery_id);
		break;
	case BATT_ID_SW3800_VC1 :
	case BATT_ID_ISL6296A_C :
		battery_profile = "tocad,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"Tocad_2500mAh for id(%lld)\n", battery_id);
		break;
	case BATT_ID_RA4301_VC0 :
	case BATT_ID_ISL6296A_N :
		battery_profile = "byd,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"BYD_2500mAh for id(%lld)\n", battery_id);
		break;
	case BATT_ID_ISL6296_C :
	case BATT_ID_ISL6296_L :
	case BATT_ID_DS2704_L :
...
#elif defined(CONFIG_LGE_PM_BATTERY_CAPACITY_4100mAh)\
	|| defined (CONFIG_LGE_USB_ID_SPDT_SWITCH)
	case LGC_4100MAH :
		battery_profile = "lgc,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"LGChem_4100mAh for id(%lld)\n", battery_id);
		break;
	case TOCAD_PANA_4100MAH :
		battery_profile = "tocad,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"Tocad _4100mAh for id(%lld)\n", battery_id);
		break;
	case ATL_4100MAH :
		battery_profile = "atl,battery-data";
		pr_info("[BATTERY PROFILE] Using battery profile - "\
				"Atl _4100mAh for id(%lld)\n", battery_id);
		break;
...
#ifdef CONFIG_LGE_USB_ID_SPDT_SWITCH
static ssize_t show_lge_battery_id(struct device *dev, struct device_attribute *attr, char *buf)
{
	int batt_id_num = read_battery_id(the_chip);

	pr_err("[LGE] show battery id : %d\n", batt_id_num);
	if(batt_id_num == LGC_4100MAH)
		return sprintf(buf,"LGC\n");
	else if(batt_id_num == TOCAD_PANA_4100MAH)
		return sprintf(buf,"TOCAD\n");
	else if(batt_id_num == ATL_4100MAH)
		return sprintf(buf,"ATL\n");
	else
		return sprintf(buf,"Unknown\n");
}
static ssize_t store_lge_battery_id(struct device *dev, struct device_attribute *attr,
		                                        const char *buf, size_t size)
{
	pr_err("[LGE] Not Support Write Fuction\n");
	return size;
}
static DEVICE_ATTR(lge_battery_id, 0644, show_lge_battery_id, store_lge_battery_id);
#endif
...
#ifdef CONFIG_LGE_USB_ID_SPDT_SWITCH
	rc = device_create_file(chip->bms_psy.dev, &dev_attr_lge_battery_id);
	if (rc < 0) {
		pr_err("Couldn't create lge_battery_id\n");
		goto fail_lge_battery_id;
	}
#endif
...
#ifdef CONFIG_LGE_USB_ID_SPDT_SWITCH
fail_lge_battery_id:
	device_remove_file(chip->bms_psy.dev, &dev_attr_lge_battery_id);
#endif
fail_get_vtg:
