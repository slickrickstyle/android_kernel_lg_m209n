...
static void rt5058_update_battery_present(struct rt5058_charger_info *ri);
...
static void rt5058_charger_otg_control(struct rt5058_charger_info *ri,
 u8 enable)
{
	int regval = 0;
	int ret;
...
static void rt5058_lock_chg_pascode(struct rt5058_charger_info *ri)
{
	...
	while (count) {
		...
		if (ret < 0)
			pr_err("%s i2c read fail ret = %02x\n", __func__, ret);
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = ri->attach_status_now;
		break;
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_TRIM:
		...
		break;
#endif
	case POWER_SUPPLY_PROP_IS_VBUS_OVP:
		val->intval = ri->is_vbus_ovp;
		break;
	default:
		ret = -EINVAL;
		break;
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		break;
	case POWER_SUPPLY_PROP_ONLINE:
		ri->attach_status_now = val->intval;
		break;
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		/* AICR */
#ifdef CONFIG_LGE_PM_USB_CURRENT_MAX
...
static int rt5058_charger_init(struct rt5058_charger_info *ri)
{
	...
	rt5058_reg_write(ri->client, RT5058_REG_CHGCTRL14, 0x33);
	rt5058_lock_chg_pascode(ri);
...
static void rt5058_update_battery_status(struct rt5058_charger_info *ri)
{
	...
	fuel_psy = power_supply_get_by_name(rt5058_fuel_devname);
	if (!fuel_psy) {
	...
	rt5058_update_battery_present(ri);

	if (value.intval >= 100 && ri->batt_present && ri->attach_status_now) {
	...
	if (ri->attach_status_now && lge_value.intval) {
		status = POWER_SUPPLY_STATUS_CHARGING;
		goto update_status;
	}
#else
...
static void rt5058_handle_plug_event(struct rt5058_charger_info *ri)
{
	...
	if (ri->otg_en) {
		pr_info("%s: otg en=1, skip plug event\n", __func__);
		return;
	}
...
#define VBUS_OVP_DQSL (20 * 1000 * 1000)
static void rt5058_handle_muic_ovp(struct rt5058_charger_info *ri)
{
    int ret, pre_is_vbus_ovp = ri->is_vbus_ovp;
    ret = rt5058_reg_read(ri->client, RT5058_REG_MUIC_STAT1);

    if (ret & RT5058_MUIC_OVP_MASK) {
		ri->is_vbus_ovp = VBUS_OVP_DQSL + 1;
		pr_err("%s: vbus ovp!!\n", __func__);
    } else {
		ri->is_vbus_ovp = 0;
	}

	if (ri->is_vbus_ovp != pre_is_vbus_ovp)
		power_supply_changed(&ri->psy);
}
...
static void rt5058_update_battery_present(struct rt5058_charger_info *ri)
{
	...
	if (pre_batt_present != ri->batt_present) {
		pr_debug("%s: battery present changed (%d)->(%d)\n",
...
static void rt5058_handle_batabs(struct rt5058_charger_info *ri)
{
	...
	/* Abnormally batt absense irq with TA W/A */
	mdelay(70);
...
static rt_irq_handler rt_chgirq_handler[CHGEVENT_MAX] = {
	...
	[MUIC_OVP_EVENT] = rt5058_chgirq_handler,
};
...
static irqreturn_t rt5058_chg_irq_handler(int irqno, void *param)
{
	...
	if (irq_ind & RT5058_MUIC_IRQMASK) {
		...
		/* after UL event(plug in/out) */
		if (regval[0] & RT5058_MUIC_OVP_MASK)
			rt_chgirq_handler[MUIC_OVP_EVENT](ri, MUIC_OVP_EVENT);
	}

	if (irq_ind & RT5058_CHG_IRQMASK) {
...
static int rt5058_charger_irqinit(struct platform_device *pdev)
{
	...
	rt5058_clr_bits(ri->client, RT5058_REG_MUIC_MASK1, RT5058_MUIC_OVP_MASK);
	return ret;
}
...
static void rt5058_charger_external_power_changed(struct power_supply *psy) {
	...
	u8 otg_en = 0;
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_CABLE_DETECT
	union lge_power_propval curr_ma, type;
#else
...
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_CABLE_DETECT
	ri->lge_cd_lpc->get_property(ri->lge_cd_lpc,
				LGE_POWER_PROP_TYPE, &type);
	otg_en = (type.intval == POWER_SUPPLY_TYPE_OTG) ? 1 : 0;
#endif

#ifdef CONFIG_LGE_PM_USB_CURRENT_MAX
	if (!ri->iinms_done && !otg_en) {
	...
	if (!ri->iinms_done)
		rt5058_set_input_current_regulation(ri, ma);
#endif
	rt5058_charger_otg_control(ri, otg_en);
...
static void lgcc_set_charging_enable(struct rt5058_charger_info *ri, int en)
{
	...
	pr_debug("%s:en/chg_enabled/attach_status_now %d/%d/%d\n",
		 __func__, en, ri->chg_enabled, ri->attach_status_now);
#ifdef CONFIG_LGE_PM_FACTORY_TESTMODE
	if (ri->testmode) {
...
static int rt5058_charger_remove(struct platform_device *pdev)
{
	...
	if (ri) {
		...
		wake_lock_destroy(&ri->safety_wake_lock);
		wake_lock_destroy(&ri->status_wake_lock);
		wake_lock_destroy(&ri->ext_psy_wake_lock);
		cancel_delayed_work(&ri->usbinsert_work);
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_VZW_REQ
		cancel_delayed_work(&ri->current_settled_work);
#endif
		rt5058_charger_irq_deinit(pdev);
		power_supply_unregister(&ri->psy);
		devm_kfree(&pdev->dev, ri);
