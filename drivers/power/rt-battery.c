...
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_BATTERY_ID_CHECKER
static int rt5058_is_valid_battery_id(struct rt_battery_info *rbi)
{
	union lge_power_propval lge_val = {0,};

	if (!rbi->batt_id_lpc) {
		rbi->batt_id_lpc = lge_power_get_by_name("batt_id");
		pr_err("batt_id lpc get failed\n");
		return 1;
	}
	rbi->batt_id_lpc->get_property(rbi->batt_id_lpc,
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VALID_BATT:
		val->intval = rt5058_is_valid_battery_id(rbi);
		break;
#else
	/* need to implment battery id check (without lpc case) */
	case POWER_SUPPLY_PROP_VALID_BATT:
		val->intval = 1;
		break;
#endif
...
static int rt_battery_remove(struct platform_device *pdev)
{
	...
	if (rbi) {
		cancel_delayed_work(&rbi->log_work);
		mutex_destroy(&rbi->param_lock);
		wake_lock_destroy(&rbi->bat_wake_lock);
