...
#else
	pr_info("No external chargin, failed to read qpnp sensor\n");
	return DEFAULT_VOLTAGE;
#endif
}
...
static int bq24262_get_prop_batt_capacity(struct bq24262_chip *chip) {
	...
	if (chip->fuelgauge != NULL) {
		...
		pr_err("%s : Failed to read externalfuel gauge\n", __func__);
...
static void update_heartbeat(struct work_struct *work)
{
	...
	if (bq24262_get_prop_batt_capacity(chip) <= 20) {
		if (bq24262_get_prop_batt_capacity(chip) <= 1) {
			pr_info("%s : SOC update per 1sec\n", __func__);
			schedule_delayed_work(&chip->update_heartbeat_work,
					round_jiffies_relative(msecs_to_jiffies
						(MIN_LOW_SOC_HEARTBEAT_MS)));
		} else {
			pr_info("%s : SOC update per 20sec\n", __func__);
			schedule_delayed_work(&chip->update_heartbeat_work,
					round_jiffies_relative(msecs_to_jiffies
						(LOW_SOC_HEARTBEAT_MS)));
		}
	} else {
		pr_info("%s : SOC update per 60sec\n", __func__);
		schedule_delayed_work(&chip->update_heartbeat_work,
				round_jiffies_relative(msecs_to_jiffies
					(UPDATE_TIME_MS)));
	}
...
static void bq24262_monitor_batt_temp(struct work_struct *work)
{
...
#ifdef CONFIG_LGE_PM_CHARGING_SAFETY_TIMER
#ifdef CONFIG_LGE_PM_PSEUDO_BATTERY
	struct power_supply *ac_psy = &chip->ac_psy;
	int rc;
	...
	if(pseudo_batt_info.mode) {
		...
		if (!ac_psy){
			...
			rc = ac_psy->set_property(ac_psy, POWER_SUPPLY_PROP_SAFETY_CHARGER_TIMER, &ret);
			if (rc < 0) {
				pr_err("%s:set safety timer disable fail\n",__func__);
			} else {
				pr_err("%s:safety_timer is disabled by pseudo_batt\n",__func__);
			}
		}
	}
#endif
...
#if CONFIG_LGE_PM_LLK_MODE
	chip->lcs_disable_chg = res.disable_chg;
#endif
...
static void bq24262_monitor_llk_mode(struct work_struct *work)
{
	...
	if (bq24262_is_charger_present(chip)) {
		...
		if (capacity > LLK_MAX_THR_SOC) {
			...
			if (chip->lcs_disable_chg) {
				pr_err("Stop charging by LLK mode(LCS disabled charging)\n");
				chip->llk_chg_enable = false;
			} else {
...
static int bq24262_get_prop_batt_present(struct bq24262_chip *chip)
{
	int temp = 0;
	bool batt_present;
#ifndef CONFIG_LGE_PM_CHARGING_BQ24262_BATT_MISS_ONLY_TEMP
	int ret = 0;
	u8 sys_status;
#endif
...
#ifdef CONFIG_LGE_PM_CHARGING_BQ24262_BATT_MISS_ONLY_TEMP
