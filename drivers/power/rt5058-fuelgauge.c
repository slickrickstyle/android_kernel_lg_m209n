...
static void rt5058_update_info(struct rt5058_fuelgauge_info *fi);
static int rt5058_fg_get_rescale_soc(int soc);
...
static void rt5058_dwork_show(struct rt5058_fuelgauge_info *fi)
{
	...
	if (!fi->suspend) {
		...
		soc = (rt5058_i2c_read_word(fi->client, RT5058_REG_FGSOC) * 10) / 256;
		resoc = rt5058_fg_get_rescale_soc(soc);
		inow = rt5058_i2c_read_word(fi->client, RT5058_REG_FGAI);
		if (inow > 32768)
			inow = inow - 65536;
		inow = inow * -1;

		if (wake_lock_active(&fi->irq_lock))
			irq_lock = 1;
		if (wake_lock_active(&fi->update_lock))
			update_lock = 1;
		pr_err("%s: %s, charigng=%d, EOC=%d, VBAT=%d, OCV=%d, SOC=%d, "
				"PRE_RESOC=%d, RESOC=%d, TEMP=%d, INOW=%d, "
				"IRQ_LOCK=%d, UPDATE_LOCK=%d\n", __func__,
				chg_status[value.intval], is_charging, is_eoc, volt, ocv,
				soc, fi->batt_soc, resoc, temp, inow, irq_lock, update_lock);
	}
	return;
}
...
unsigned int fg_get_soc(struct rt5058_fuelgauge_info *fi)
{
...
#if RT5058_FG_DEBUG
	pr_err("%s: vgcomp_input: avg_volt=%d, temp=%d, curr=%d\n",
			__func__, avg_volt, fi->temperature, curr);
#endif
	vgcomp = rt5058_fg_get_vgcomp(fi, avg_volt, fi->temperature, curr);
#if RT5058_FG_DEBUG
	pr_err("%s: vgcomp_output: vgcomp[0~8] = %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
			__func__, vgcomp.data[0],vgcomp.data[1],vgcomp.data[2],vgcomp.data[3],
			vgcomp.data[4],vgcomp.data[5],vgcomp.data[6],vgcomp.data[7],vgcomp.data[8]);
#endif
	...
	ret = rt5058_i2c_read_word(fi->client, RT5058_REG_FGSOC);
	if (ret < 0) {
	...
	pr_err("%s: VBAT=%d, OCV=%d, SOC_OFS=%d, SOC=%d, PRE_RESOC=%d, RESOC=%d, "
			"SOCCFG=0x%04x, temp=%d, INOW=%d, char=%d, eoc=%d\n",
			__func__, avg_volt, ocv, offset,
			soc_val, fi->batt_soc, rt5058_fg_get_rescale_soc(soc_val),
			rt5058_i2c_read_word(fi->client, RT5058_REG_FGSOCCFG1),
			fi->temperature, inow, is_charging, rt5058_fg_is_eoc(fi));
	return soc_val;
}
...
static int rt5058_fg_init(struct rt5058_fuelgauge_info *fi)
{
	...
	if (fi->fdata->use_sc_count) {
		...
		fi->pre_raw_soc = rt5058_i2c_read_word(client, RT5058_REG_FGSOC);
		pr_info("fi->pre_raw_soc = %d\n", fi->pre_raw_soc);
		/* init soc set */
		fi->batt_soc = rt5058_fg_get_rescale_soc((fi->pre_raw_soc * 10) / 256);
	...
	if (fi->fdata->battery_type == 4400) {
		pr_info("%s: set 4400 Battery OCV Table\n", __func__);
		...
		pr_info("%s: set 4200 Battery OCV Table\n", __func__);
		rt5058_i2c_write_word(client, RT5058_REG_FGMFA, 0x8085);
		mdelay(5);
		ret = rt5058_i2c_read_word(client, RT5058_REG_FGMFA);
		ret &= ~0x0009;
			...
			struct rt5058_fuelgauge_platform_data *fdata)
{
	...
	const char* battery_profile = "rt,battery_fg_comp_data";
	int batt_id = 0;
#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
	...
	case BATT_ID_RA4301_VC0:
		...
		break;
#elif defined CONFIG_LGE_PM_BATTERY_RT5058_LGIP530A_1100mAh
	case BATT_ID_SW3800_VC0:
		battery_profile = "rt,tocad_battery_fg_comp_data";
		pr_err("%s: tocad_battery_fg_comp_data\n", __func__);
		break;
#elif defined CONFIG_LGE_PM_BATTERY_RT5058_BL45F1F_2500mAh
	case BATT_ID_SW3800_VC0:
		battery_profile = "rt,lgc_battery_fg_comp_data";
		pr_err("%s: lgc_battery_fg_comp_data\n", __func__);
		break;
#endif
	default:
		battery_profile = "rt,battery_fg_comp_data";
		pr_err("%s: unknown, battery_fg_comp_data\n", __func__);
		break;
	}

	/* set fg_soc_table */
#if defined CONFIG_LGE_PM_BATTERY_RT5058_3P2_OCV_TABLE_USED
	fg_soc_table = "rt,fg_soc_table_3_2v";
#elif defined CONFIG_LGE_PM_BATTERY_RT5058_3P3_OCV_TABLE_USED
	fg_soc_table = "rt,fg_soc_table_3_3v";
#elif defined CONFIG_LGE_PM_BATTERY_RT5058_3P4_OCV_TABLE_USED
...
static void rt5058_fg_avg_quick_sensing(struct rt5058_fuelgauge_info *fi)
{
	...
	fi->batt_soc = rt5058_fg_get_rescale_soc(fg_get_soc(fi));
	/* call power_supply_changed */
	power_supply_changed(&fi->psy);
}
#endif

static void rt5058_update_work_func(struct work_struct *work)
{
	struct rt5058_fuelgauge_info *fi;
	fi = container_of(work, struct rt5058_fuelgauge_info,
			update_work.work);
	...
	if (fi->suspend) {
		pr_err("%s: suspend, run update_work_func 20msec again\n", __func__);
		schedule_delayed_work(&fi->update_work, msecs_to_jiffies(20));
	} else {
		rt5058_update_info(fi);
	}
	return;
}
...
{
	wake_lock(&fi->irq_lock);
	if (fi->suspend) {
		pr_err("%s: run update_worker after 20msec\n", __func__);
		schedule_delayed_work(&fi->update_work, msecs_to_jiffies(20));
	} else {
		rt5058_update_info(fi);
	}
...
static void rt5058_handle_under_voltage(struct rt5058_fuelgauge_info *fi)
{
	...
	if (ret & RT5058_FGUV_MASK) {
		if (value.intval) {
			pr_info("%s: Force Recharge by S/W\n", __func__);
...
#define RT5058_SOC_RESCAILING 93
static int rt5058_fg_get_rescale_soc(int soc)
{
	int resoc;
	...
	if (soc <= 50) {
		...
		resoc = (soc*10000)/RT5058_SOC_RESCAILING/1000;
	}
...
#define SOC_EXCEPTION_PERIOD_BASE 30000 // 30 sec
static void rt5058_update_info(struct rt5058_fuelgauge_info *fi)
{
	int raw_soc;
	int pre_raw_soc;
	...
	if (fi->update_lock_flag)
		pr_err("%s: exception, try update again\n", __func__);
	else
		wake_lock(&fi->update_lock);

	/* wake_unlock irq_lock */
	if (wake_lock_active(&fi->irq_lock))
		wake_unlock(&fi->irq_lock);

	raw_soc = rt5058_i2c_read_word(fi->client, RT5058_REG_FGSOC);
	soc = raw_soc / 256;

	pre_raw_soc = fi->pre_raw_soc;
	pre_soc = pre_raw_soc / 256;

	resoc = rt5058_fg_get_rescale_soc(fg_get_soc(fi));
	pre_resoc = fi->batt_soc;

	diff_soc = abs(resoc - pre_resoc);

	if (diff_soc > 1) {
		fi->update_lock_flag = true;
		pr_err("%s: soc update exception!, weight:%d\n", __func__, diff_soc);
		if (resoc > pre_resoc) {
			update_soc = pre_resoc + 1;
		} else {
			update_soc = pre_resoc - 1;
		}
		cancel_delayed_work(&fi->update_work);
		diff_soc = (diff_soc > 4) ? SOC_EXCEPTION_PERIOD_BASE : diff_soc;
		PERIOD = SOC_EXCEPTION_PERIOD_BASE + SOC_EXCEPTION_PERIOD_BASE/diff_soc;
		schedule_delayed_work(&fi->update_work, msecs_to_jiffies(PERIOD));
	} else {
		update_soc = resoc;
		fi->update_lock_flag = false;
	}

	if (fi->update_lock_flag == false) {
		if (wake_lock_active(&fi->update_lock)) {
			wake_unlock(&fi->update_lock);
		}
	}
	fi->pre_raw_soc = raw_soc;
	fi->batt_soc = update_soc;
	pr_err("%s: soc=%d, pre_soc=%d, resoc=%d, pre_resoc=%d, new_batt_soc=%d\n",
			__func__, soc, pre_soc, resoc, pre_resoc, fi->batt_soc);
	power_supply_changed(&fi->psy);
	return;
}
#ifdef CONFIG_LGE_PM_FACTORY_TESTMODE
...
static int rt5058_fuelgauge_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&fi->update_lock, WAKE_LOCK_SUSPEND, "rt5058_fg_update_lock");
	wake_lock_init(&fi->irq_lock, WAKE_LOCK_SUSPEND, "rt5058_fg_irq_lock");
	INIT_DELAYED_WORK(&fi->update_work,
			rt5058_update_work_func);
	pr_err("%s: start update worker\n", __func__);
...
#if RT5058_FG_DEBUG
		cancel_delayed_work(&fi->dwork);
		wake_lock_destroy(&fi->dwork_lock);
#endif
		mutex_destroy(&fi->param_lock);
		cancel_delayed_work(&fi->update_work);
		wake_lock_destroy(&fi->irq_lock);
		wake_lock_destroy(&fi->update_lock);
