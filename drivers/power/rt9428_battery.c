...
#elif defined (CONFIG_MSM8909_K6B)
#define RT9428_SOC_RESCALING 90
#else
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_RT9536_SUPPORT
			batt_volt = rt9428_get_vcell(ref);
			i = rt9428_get_capacity(ref);

			if (lge_get_factory_boot()) {
				if((batt_volt > 4300) && (i > 90)){
					i = 100;
				}
			}
#else
...
static void rt9428_update_info(struct rt9428_chip *chip)
{
	...
	if (regval < 0) {
		pr_err("read config fail(%d). set config to default 0x325f\n", regval);
		regval = 0x325f;
	} else {
		regval &= (~RT9428_SOCALRT_MASK);
		regval |= RT9428_SCEN_MASK;
		regval &= (~RT9428_SLEEP_MASK);
	}
	...
	/*adjust vgcom */
	{
		...
		if(capacity_raw < 0)
		{
			if (wake_lock_active(&chip->update_lock))
				wake_unlock(&chip->update_lock);
				pr_err("%s:read soc read fail, release update_lock\n", __func__);
				return;
		}
...
static void rt9428_polling_work(struct work_struct *work)
{
	...
	if (!wake_lock_active(&chip->polling_lock)){
		pr_err("%s: acquire polling lock >>> \n",__func__);
		wake_lock(&chip->polling_lock);
	}

	rt9428_reg_info(chip);

#if defined(CONFIG_LGE_PM_BATTERY_RT9428_SC_ALERT_RECOVER)
	gpio_val = gpio_get_value(chip->pdata->alert_gpio);
	pr_err("FUEL_INT %c\n",gpio_val?'H':'L');
#endif
...
#endif
#if defined(CONFIG_MSM8909_K6B) || defined(CONFIG_MACH_MSM8909_LV1_GLOBAL_COM)
	raw_capacity = chip->last_raw_capacity;
#endif
	status_val = rt9428_reg_read_word(chip->i2c, RT9428_REG_STATUS);
	if (status_val < 0) {
		pr_err("read status fail(%d). set status to default 0x00\n", status_val);
		goto reschedule_work;
		status_val &= 0x00FF;
		}
	...
	if( 0
#ifdef CONFIG_LGE_PM_BATTERY_RT9428_RESET_RECOVER
	|| (status_val&0x0100)||(!(config_val&0x0040))
#endif
#ifdef CONFIG_LGE_PM_RT9428_ABNORMAL_SLEEP_MODE
	|| (config_val&0x0080)
#endif
#ifdef CONFIG_LGE_PM_BATTERY_RT9428_SC_ALERT_RECOVER
	|| (status_val&0x2000)||(config_val&0x0020)
#endif
	)
	{
		/* CHECK [RI]@Reg-Status is set OR [SCEN]@Reg-Config is cleared */
		if((status_val&0x0100)||(!(config_val&0x0040))) {
			pr_err("%s:fg reset occur\n", __func__);
		}
		/* CHECK [Sleep]@Reg-Config is set */
		if (config_val&0x0080) {
			pr_err("%s:fg enter sleep mode\n", __func__);
		}
		/* CHECK [Sleep]@Reg-Config is set OR [Alert]@Reg-Config is set*/
		if ((status_val&0x2000)||((config_val&0x0020))) {
			pr_err("%s:fg SCEN or alert bit set\n", __func__);
		}
		/* get Alert pin GPIO status */
		gpio_val = gpio_get_value(chip->pdata->alert_gpio);
		pr_err("FUEL_INT %c\n",gpio_val?'H':'L');
		...
		/* clear Alert bit anyway then set [SCEN] bit */
		config_val &= (~RT9428_SOCALRT_MASK);
		config_val |= RT9428_SCEN_MASK;
		/* clear [Sleep] bit */
		config_val &= (~RT9428_SLEEP_MASK);

		rc= rt9428_reg_write_word(chip->i2c, RT9428_REG_STATUS, status_val);
		if (rc) {
			pr_err("Unable to set 'STATUS' rc=%d\n", rc);
		}
		...
		rc = rt9428_reg_read(chip->i2c, RT9428_REG_SOCH);
		if (rc < 0) {
			pr_err("read soc fail(%d), skip soc-backup \n", rc);
		}
		else{
			pre_raw_soc_val = rc;
		}
...
end_work:
	if (wake_lock_active(&chip->polling_lock)){
		pr_err("%s: release polling lock >>> \n",__func__);
		wake_unlock(&chip->polling_lock);
	}
}
#endif
...
static int rt_parse_dt(struct device *dev, struct rt9428_platform_data *pdata)
{
	...
	switch ( batt_id ){
		...
		case BATT_ID_DS2704_C:
			...
			break;
#elif defined(CONFIG_LGE_PM_BATTERY_RT9428_BL41A1HB_2100mAh)
			/* K6B */
		...
		case BATT_ID_SW3800_VC1:
			profile_id = RT9428_BATTERY_TOCAD;
			break;
#elif defined(CONFIG_LGE_PM_BATTERY_RT9428_BL45A1H_2300mAh)
		/* M2 */
		case BATT_ID_SW3800_VC0:
		case BATT_ID_RA4301_VC1:
			profile_id = RT9428_BATTERY_LGC;
		...
		case BATT_ID_OPEN_LGC:
			profile_id = RT9428_BATTERY_LGC;
			break;
#elif defined(CONFIG_LGE_PM_BATTERY_RT9428_BL45F1F_2500mAh)
		case BATT_ID_SW3800_VC0:
		case BATT_ID_RA4301_VC1:
			profile_id = RT9428_BATTERY_LGC;
			break;
	...
	if (of_property_read_u32_array(np_profile,
		...
		pr_err("%s:check vgpara_hcap:0x%x 0x%x 0x%x 0x%x 0x%x\n", __func__,
				prop_array[0], prop_array[1], prop_array[2], prop_array[3], prop_array[4]);
	...
	if (of_property_read_u32_array(np_profile,
		...
		pr_err("%s:check vgpara_lcap:0x%x 0x%x 0x%x 0x%x 0x%x\n", __func__,
				prop_array[0], prop_array[1], prop_array[2], prop_array[3], prop_array[4]);
...
#endif
#ifdef CONFIG_LGE_PM_RT9428_POLLING
	wake_lock_init(&chip->polling_lock, WAKE_LOCK_SUSPEND, "rt9428_polling_lock");
...
#endif
#ifdef CONFIG_LGE_PM_RT9428_POLLING
	wake_lock_destroy(&chip->polling_lock);
#endif
err_init:
...
#endif
#ifdef CONFIG_LGE_PM_RT9428_IRQ_LOW_WA
	cancel_delayed_work_sync(&chip->update_polling_work);
#endif
