...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (cmd != NULL) {
		if (!strncmp(cmd, "bootloader", 10)) {
...
#ifdef CONFIG_LGE_USE_PON_REBOOT_REG
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_LGE_FOTA_LCD_OFF);
#endif
...
#ifdef CONFIG_LGE_USE_PON_REBOOT_REG
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_LGE_FOTA_OUT_LCD_OFF);
#endif
...
#ifdef CONFIG_LGE_USE_PON_REBOOT_REG
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_LGE_WALLPAPER_FAIL);
#endif
			__raw_writel(0x77665507, restart_reason);
		} else if (!strcmp(cmd, "dm-verity device corrupted")) {
#ifdef CONFIG_LGE_USE_PON_REBOOT_REG
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_DM_VERITY_LOGGING);
#endif
			...
			if (!ret) {
#ifdef CONFIG_LGE_USE_PON_REBOOT_REG
				if ((0x6f656d00 | (code & 0xff)) == LAF_RESTART_MODE) {
					qpnp_pon_set_restart_reason(
						PON_RESTART_REASON_LGE_LAF_RESTART_MODE);
				}
#endif
...
#ifdef CONFIG_LGE_USE_PON_REBOOT_REG
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_LGE_REBOOT_NORMAL);
#endif
