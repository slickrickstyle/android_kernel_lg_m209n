...
#define LLK_MODE_POLLING_PERIOD 60
#endif
...
static struct debug_reg qpnp_lbc_debug_regs[] = {
	...
	QPNP_LBC_DEBUG_REG(CHG_IBAT_MAX_REG),
};
...
static int get_prop_capacity(struct qpnp_lbc_chip *chip)
{
	union power_supply_propval ret = {0,};
	int soc;
	static int pre_soc = DEFAULT_CAPACITY;
...
#ifndef CONFIG_LGE_PM_THERMAL_VTS
skip_current_config:
#endif
...
void lgcc_charger_reginfo(void)
{
	...
	pr_info("%s=0x%02X batt_volt = %d, batt_temp = %d\n", qpnp_lbc_debug_regs[4].name, reg_val[4],
	...
	case POWER_SUPPLY_PROP_STORE_DEMO_ENABLED:
		if (chip->store_demo_enabled ^ val->intval) {
			...
			if (chip->store_demo_enabled) {
				...
				schedule_delayed_work(&chip->llkmode_work, 1*HZ);
			} else {
				pr_info("stop llkmode work\n");
				cancel_delayed_work_sync(&chip->llkmode_work);
				qpnp_lbc_charger_enable(chip, LLK, 1);
			}
		}
		break;
#endif
...
#ifdef CONFIG_LGE_PM_LLK_MODE
static void lbc_llkmode_work(struct work_struct *work)
{
	struct qpnp_lbc_chip *chip =
		container_of(work, struct qpnp_lbc_chip, llkmode_work.work);
	int capacity = 0;
	union power_supply_propval ret = {0,};
	chip->batt_psy.get_property(&chip->batt_psy,
			POWER_SUPPLY_PROP_CAPACITY, &ret);
	...
	if (qpnp_lbc_is_usb_chg_plugged_in(chip)) {
		if (capacity > LLK_MAX_THR_SOC) {
			qpnp_lbc_charger_enable(chip, LLK, 0);
			pr_info("Stop charging by LLK mode\n");
		} else if (capacity < LLK_MIN_THR_SOC) {
			qpnp_lbc_charger_enable(chip, LLK, 1);
			pr_info("Start charging by LLK mode\n");
		}
	}
	schedule_delayed_work(&chip->llkmode_work, LLK_MODE_POLLING_PERIOD*HZ);
}
#endif
static void lbc_power_off_work(struct work_struct *work)
{
	struct qpnp_lbc_chip *chip =
		container_of(work, struct qpnp_lbc_chip, power_off_work.work);
	if (!qpnp_lbc_is_batt_present(chip) && !is_factory_cable()) {
		pr_info("battery missing, trigger power off in worker\n");
		kernel_power_off();
	}
}
...
static irqreturn_t qpnp_lbc_batt_pres_irq_handler(int irq, void *_chip)
{
	...
	if (chip->batt_present ^ batt_present) {
		...
		if (!chip->batt_present && !is_factory_cable()) {
			pr_info("battery missing trigger, start power_off work\n");
...
#endif
#ifdef CONFIG_LGE_PM_LLK_MODE
	INIT_DELAYED_WORK(&chip->llkmode_work, lbc_llkmode_work);
#endif
	INIT_DELAYED_WORK(&chip->power_off_work, lbc_power_off_work);
