 ...
 *
 *    File  	: lgtp_device_ft8606.h
 *    Author(s)   : 
 *    Description :
 *
 ***************************************************************************/

#if !defined ( _LGTP_DEVICE_FT8606_H_ )
#define _LGTP_DEVICE_FT8606_H_
...
extern ssize_t FT8606_GetTestResult(struct i2c_client *client, char *pBuf, int *result, int type);
extern void log_file_size_check(struct device *dev);
extern void FT8606_WriteFile(char *filename, char *data, int time);
extern void FT8606_Reset(int status, int delay);
extern int FT8606_change_mode(struct i2c_client *client, const u8 mode);
